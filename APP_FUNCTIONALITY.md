...
- The service layer in `app-backend/` is retained; the retired TrainingPeaks MCP source in `src/` has been removed.

## Product purpose

The app is an athlete-facing triathlon coaching workspace. It combines a 90-day training history, the current Intervals.icu plan, recovery and performance signals, concise AI coaching, controlled workout adjustments, workout feedback, and connection settings. The current coaching phase is taper for IRONMAN 70.3 Waco.
...
| `/settings`             | Connection status and credential entry for Intervals.icu, OpenAI, and Supabase; athlete zones; race details; notification preferences; reconnect/save actions. Never return stored secrets to the browser.      |
...
- Preserve functional credential entry for Intervals.icu, OpenAI, and Supabase; athlete zones and race context; theme selection; and workout-notification timing and delivery controls.
...
- Place equal-width HRV and resting-heart-rate cards side by side to its upper right, with one Intervals.icu card beneath them.
...
- In the Intervals.icu card, show Fitness/CTL, Fatigue/ATL, and Form/TSB as three side-by-side units.
- Show Intervals.icu fitness/CTL with fatigue/ATL and form/TSB beneath it.
...
- On mobile, keep the workout and Intervals.icu cards full width, place HRV and resting heart rate side by side, reduce chart height without removing data, and preserve touch-accessible tooltips.
...
2. Any AI-proposed Intervals.icu or workout change must clearly show the current plan, proposed change, reason, and risk level.
...
| `POST /api/config`                                        | Optional `INTERVALS_API_KEY`, `OPENAI_API_KEY`, `SUPABASE_URL`, `SUPABASE_SECRET_KEY` | Merges non-empty values into local configuration and returns connection booleans.                                                                                  |
...
| `POST /api/daily-reviews/:id/approve`                     | —                                                                                  | Revalidates the live schedule, recovery, and workout state, applies the exact displayed patch to Intervals.icu, then verifies the write.                           |
| `POST /api/daily-reviews/:id/deny`                        | —                                                                                  | Records denial and leaves Intervals.icu unchanged.                                                                                                                 |
| `POST /api/start`                                         | —                                                                                  | Starts the compiled Intervals.icu MCP child process.                                                                                                               |
...
interface CoachDecision {
  ...
  reason: string;
  recommendation: string;
  proposed_intervals_change: string | null;
...
1. With `INTERVALS_API_KEY`, the service exchanges the Intervals.icu cookie for a short-lived bearer token and fetches workouts, performance, and wellness data.
2. Live Intervals.icu context is cached in memory for five minutes and on disk for degraded operation.
3. If live sync fails, return the Intervals.icu disk cache plus local comments/library and a `sync_error`.
4. If no cache is usable, return local context plus the sync error.
5. Without Intervals.icu credentials, return local context with source `local-live`.
...
- Ground coaching answers in the live Intervals.icu context, falling back to the Intervals.icu disk cache before local seed data. Resolve relative dates against the current date and use completed duration/load for completed-workout reviews.

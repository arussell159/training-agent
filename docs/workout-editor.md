...
The application's description is a separate, read-only projection of the saved structured workout. Save/readback, sync, and cached context projection generate plain **Warm Up**, **Main Set**, and **Warm Down** sections with one distinct interval or repeat per line. Identical prescriptions can group; progressions, omitted final recoveries, and recovery between sets retain their execution order. Bike/run time uses mins, rests use secs, and swimming distance uses yd with the prescribed stroke/equipment. Zone labels use the athlete's configured boundaries; exact absolute prescriptions remain alongside inferred zone labels. Missing boundaries never produce guessed zones. General coaching prose is excluded from this reference view.

This formatter changes only the app's description text. It never calls a provider mutation, modifies the native serializer, or changes the saved structure, targets, timing, distance, sport, or source description. Original provider data and editor notes remain intact. A version marker preserves already generated descriptions when compact app caches remove raw provider data and zone settings. Unsupported or unstructured workouts retain their existing description.
...
Mobile step cards align the title, measure/shape row, duration/targets, and full-width zone selector, with Add step and the step actions on a separate bottom row. Repeat counts remain centered and readable. Description tests cover all three sports, identical and mixed repeats, swim combinations, progression and recovery differences, cached projection, and provider immutability.

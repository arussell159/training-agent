...
- The display-unit and cadence-goal controls are removed. Existing cadence prescriptions remain intact when another field is edited. **Units** converts compatible prescriptions; incompatible choices start a new prescription and prompt for the intended values. Power percentages require a known FTP for absolute conversion. Pace export always includes a unit.
- Time fields use hh:mm:ss. Measure and target shape sit beside the step title. Training-zone dropdowns load the actual saved sport boundaries from Intervals.icu; closed zones fill editable numeric ranges, while open-ended zones retain native zone prescriptions without invented limits. The provider defines [zone boundaries as percentages of threshold](https://forum.intervals.icu/t/server-side-data-model-for-scripts/25781).
...
Release checks also cover: a 180-second main interval versus a 180-second aggregate repeat made of 30-second leaves; calendar-day draft creation; one verified POST; uncertain-create reconciliation; and editing the created event. The isolated release passes all 160 backend tests.

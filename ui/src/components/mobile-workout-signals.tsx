...
type Lap={id:string;label:string;start:number;end:number}
export function MobileWorkoutSignals({points,laps,duration,sport}:{points:RecordedPoint[];laps:Lap[];duration:number;sport:string}){
 ...
 const move=(e:PointerEvent<SVGSVGElement>)=>{const b=e.currentTarget.getBoundingClientRect();setTime(Math.max(0,Math.min(duration,((e.clientX-b.left)/b.width*400-40)/344*duration)))}
 ...
 return <section className="space-y-5 md:hidden" aria-label="Recorded workout graphs">
  <div><p className="mb-2 text-xs font-medium text-muted-foreground">{lap?.label || 'Laps'}</p><div className="relative h-8" style={{marginLeft:'10%',marginRight:'4%'}}>{laps.map((l,i)=><button key={l.id} aria-label={`Highlight ${l.label}`} aria-pressed={lap?.id===l.id} onClick={()=>{setLap(l);setTime(l.start)}} style={{left:`${Math.max(0,l.start)/duration*100}%`,width:`${Math.max(0,Math.min(duration,l.end)-Math.max(0,l.start))/duration*100}%`}} className={`absolute inset-y-0 overflow-hidden rounded border border-background text-[10px] ${lap?.id===l.id?'bg-sky-500 text-white':'bg-slate-200 text-slate-700'}`}>{i+1}</button>)}</div></div>
  {tracks.map(key=>{
   ...
   return <div key={key} className="relative"><h3 className="mb-1 text-base font-semibold">{label(key)}</h3>
    {nearest&&<div role="status" className="pointer-events-none absolute right-2 top-7 z-[999] rounded-lg border bg-white px-3 py-2 text-xs text-slate-900 shadow-lg"><p className="mb-1 text-slate-500">{clock(nearest.time)}{laps.find(l=>nearest.time>=l.start&&nearest.time<l.end)?` · ${laps.find(l=>nearest.time>=l.start&&nearest.time<l.end)?.label}`:''}</p><strong>{format(value(nearest,key),key)} {unit(key)}</strong></div>}

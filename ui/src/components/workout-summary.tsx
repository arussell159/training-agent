...
export function WorkoutSummary({workout,showElapsed=true,embedded=false,section='all'}:{workout:PlannedWorkout;showElapsed?:boolean;embedded?:boolean;section?:'all'|'overview'|'recorded'}){
 ...
 const pace=(v:number|null|undefined,recorded=true)=>{if(!(Number(v)>0))return '';const s=Math.round((swim?(recorded?METERS_PER_100_YARDS:100):1609.344)/Number(v));return `${Math.floor(s/60)}:${String(s%60).padStart(2,'0')}`}
 const rows:{label:string;unit:string;value:(v:WorkoutSummaryValues|undefined|null,recorded?:boolean)=>string}[]=[
  ...
  ...(showElapsed && /swim|run/i.test(workout.sport)?[
    ...
    {label:'Elapsed pace',unit:swim?'min:sec/100y':'min/mi',value:(v:WorkoutSummaryValues|undefined|null,recorded=false)=>pace(v?.elapsed_speed,recorded)},
  ]:[]),
  {label:'Distance',unit:swim?'yds':'mi',value:(v,recorded=false)=>number(v?.distance_meters==null||v.distance_meters<=0?null:v.distance_meters/(swim?(recorded?METERS_PER_YARD:1):1609.344),swim?0:2)},
  {label:bike?'Avg power':'Avg moving pace',unit:bike?'watts':swim?'/100 yd':'/mi',value:(v,recorded=false)=>bike?number(v?.average_power):pace(v?.average_speed,recorded)},
 ...
 const visible=rows.filter(row=>row.value(planned)!=='' || row.value(completed,true)!=='')
 const recordedRows=[
 ...
 return <section aria-label="Planned and completed" className={`w-full space-y-3 ${embedded?'rounded-xl border bg-card p-4 shadow-sm':'mx-auto md:max-w-[440px]'}`}>
  {section!=='recorded'&&!!visible.length&&<div className={`overflow-hidden ${embedded?'':'border'}`}><table className="w-full table-fixed text-xs">
   ...
   <tbody>{visible.map(row=><tr key={row.label} className={green+' border-t border-border/50'}><th className="px-3 py-1.5 text-left font-normal">{heading(row.label,row.unit)}</th><td className="whitespace-nowrap px-1 py-1.5 text-center text-sm font-medium tabular-nums">{row.label==='Duration'&&workout.planned_time_label || row.value(planned)}</td><td className="whitespace-nowrap px-1 py-1.5 text-center text-sm font-medium tabular-nums">{row.value(completed,true)}</td></tr>)}</tbody>
  </table></div>}

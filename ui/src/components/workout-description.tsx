...
export function WorkoutDescription({workout,title='Workout instructions',mobileCompact=false}:{workout:PlannedWorkout;title?:string;mobileCompact?:boolean}){
 ...
 return <section className={mobileCompact?'space-y-2':'space-y-3'} aria-label="Workout description"><div className="flex items-center justify-between"><h3 className={mobileCompact?'text-base font-bold':'text-sm font-semibold'}>{title}</h3>{!editing&&<Button variant="ghost" size="sm" className={mobileCompact?'size-7 p-0':''} onClick={()=>{setDraft(saved);setEditing(true)}} aria-label="Edit workout details"><Pencil className="size-3.5"/>{mobileCompact?<span className="sr-only">Edit</span>:'Edit'}</Button>}</div>

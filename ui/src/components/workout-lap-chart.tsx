...
export function WorkoutLapChart({points,laps,sport,selected,onSelect}:{points:RecordedPoint[];laps:RecordedLap[];sport:string;selected:RecordedLap|null;onSelect:(lap:RecordedLap)=>void}){
  ...
  const duration=Math.max(1,...laps.map(lap=>lap.end))
  const width=Math.max(560,laps.length*48)
  ...
  const x=(time:number)=>Math.max(0,time)/duration*width
  ...
  const select=(lap:RecordedLap,clientX?:number)=>{
    ...
    if(clientX!=null)position(clientX-viewport.getBoundingClientRect().left)
      ...
      const center=x((lap.start+lap.end)/2)
  ...
  return <section aria-label="Lap chart" className="space-y-2 md:hidden">
    ...
    <div className="relative pt-[76px]">
      ...
      <div className="relative ml-12">
        ...
        <div ref={scroll} className="overflow-x-auto overscroll-x-contain touch-pan-x pb-1" onScroll={()=>{if(selected&&scroll.current)position(x((selected.start+selected.end)/2)-scroll.current.scrollLeft)}}>
          <svg width={width} height="228" viewBox={`0 0 ${width} 228`} className="block select-none" role="group" aria-label="Scrollable interval averages">
            ...
            {bars.map(bar=>{
              const left=x(bar.lap.start),barWidth=Math.max(2,x(bar.lap.end)-left-3),top=y(bar.value)
              return <g key={bar.lap.id} role="button" tabIndex={0} aria-label={`${bar.lap.label}, ${format(bar.value)} ${unit}, duration ${formatSignalClock(bar.lap.end-bar.lap.start)}`} aria-pressed={selected?.id===bar.lap.id} className="cursor-pointer outline-none focus:opacity-70" onClick={event=>select(bar.lap,event.clientX)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();select(bar.lap)}}}>
      ...
      <p className="ml-12 text-[10px] text-muted-foreground">{unit} · Bar width shows interval duration</p>
    </div>
  </section>
}

...
test("pre presentation preserves exact readiness, missing data and priority", () => {
  const report = parseWorkoutReport(
    "Recommendation: Modify\nDecision Level: P1\nHRV: Unavailable (baseline 49.25 ms)\nSleep: 7.13 h\nInterpretation:\nMissing current HRV; do not increase intensity.",
    "pre"
  );
...
test("source summary leads the post report without converting it to a metric", () => {
  const report = parseWorkoutReport(
    "Data (last_updated UTC: 2026-09-18T11:36:32)\nTwo same-day sessions completed: the swim was truncated versus plan.\nCompleted workout: Swim CSS\nDuration: 32m57s (planned 47m21s)\nHR zones: 10% Zone 1, 90% Zone 2",
    "post"
  );
  assert.deepEqual(report.verdict, [
    ["Session Result", "Two same-day sessions completed: the swim was truncated versus plan."],
  ]);

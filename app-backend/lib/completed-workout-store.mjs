...
export function providerConnection(config) {
  return createHash("sha256")
    .update(`${config.SETTINGS_SCOPE || "default"}:${config.INTERVALS_API_KEY || ""}`)
    .digest("hex");
}
...
export function createCompletedWorkoutStore(config, store) {
  ...
  async function metadata(id) {
    return store.getSyncRecord(recordId(id, "metadata"));
  }
  return {
    ready: store.ready,
    ...
    async load(id, kind, download, { force = false } = {}) {
      ...
      if (saved && saved.revision === revision && Object.hasOwn(saved, "data"))
        return saved.encoding === "gzip-json-v1"
          ? JSON.parse(gunzipSync(Buffer.from(saved.data, "base64")).toString("utf8"))
          : saved.data;
      ...
      if (store.ready) {
        ...
        await write(
          ...
          json.length > 65536
            ...
            : { revision, data, saved_at: new Date().toISOString() }
        );
      }
      return data;
    },
...
export function mergeTrainingSnapshot(previous, incoming, range) {
  ...
  const merge = (oldRows = [], newRows = []) => {
    const recent = new Map(
      oldRows
        .filter(
          (w) =>
            ["pending", "failed"].includes(w.sync_status) ||
            (incoming.sync_started_at && w.app_updated_at > incoming.sync_started_at)
        )
        .map((w) => [String(w.id), w])
    );
    return [
          ...
          ...newRows.filter(
            (w) =>
              !(
                ...
                previous.app_deleted_workouts?.[String(w.id)] > incoming.sync_started_at
              )
          ),
          ...recent.values(),
        ].map((w) => [String(w.id), w])
  ...
  const performance = new Map(
    [...(previous.performance || []), ...(incoming.performance || [])].map((w) => [w.workoutDay, w])
  );
  return {
    ...previous,
    ...incoming,
    ...
    performance: [...performance.values()].sort((a, b) => a.workoutDay.localeCompare(b.workoutDay)),
  };
}

export function snapshotCoversRange(snapshot, range) {
  if (!range) return true;
  return (snapshot.cached_ranges || []).some(
    (saved) => saved.start <= range.start && saved.end >= range.end
  );
}

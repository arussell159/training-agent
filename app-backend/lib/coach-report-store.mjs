...
export const COACH_REPORT_KINDS = ["pre_workout", "post_workout", "weekly", "block"];

export function reportTitle(kind, sport, title) {
  if (kind === "pre_workout" || kind === "post_workout") {
    const raw = String(sport || "").trim();
    if (!raw || raw.length > 80) throw new CoachError("Choose a sport for the workout report.", 400);
    const aliases = { cycling: "Bike", ride: "Bike", running: "Run", swimming: "Swim" };
    const label = aliases[raw.toLowerCase()] || raw.replace(/\b\w/g, letter => letter.toUpperCase());
  ...
  return supplied || (kind === "weekly" ? "Weekly Report" : "Block Report");
}

export function normalizeCoachReport(input, scope = "default", now = () => new Date()) {
  if (!input || typeof input !== "object" || Array.isArray(input) || !COACH_REPORT_KINDS.includes(input.kind))
  ...
  if (!validReportDate(startDate) || !validReportDate(endDate) || endDate < startDate ||
  ...
  if (!body.trim() || body.length > 250_000) throw new CoachError("Report text is required and must be under 250,000 characters.", 400);
  const identifier = value => {
    const text = String(value || "").trim();
    if (text && !/^[\w:.-]{1,150}$/.test(text)) throw new CoachError("Invalid report source identifier.", 400);
    return text || null;
  };
  ...
  const sourceKey = identifier(input.sourceKey) || (
    kind.endsWith("_workout")
      ? `${kind}:${workoutId || eventId || activityId || ""}`
      : kind === "weekly" ? `weekly:${startDate}` : `block:${startDate}:${endDate}`
  );
  ...
  if (!Number.isFinite(Date.parse(generatedAt))) throw new CoachError("Invalid report generation time.", 400);
  ...
  return {
    id, scope, athlete_id: athleteId, kind, sport, workout_id: workoutId,
    event_id: eventId, activity_id: activityId, plan_id: planId,
    start_date: startDate, end_date: endDate, title, body,
    generated_at: new Date(generatedAt).toISOString(), source_key: sourceKey,
...
function publicReport(row) {
  return {
    id: row.id, kind: row.kind, sport: row.sport, workoutId: row.workout_id,
    eventId: row.event_id, activityId: row.activity_id, planId: row.plan_id,
    startDate: row.start_date, endDate: row.end_date, title: row.title,
    text: row.body, generatedAt: row.generated_at, sourceKey: row.source_key,
    source: "app",
  };
}
...
export function createCoachReportStore(bootstrap, fetchImpl = fetch, now = () => new Date()) {
  ...
  return {
    async list() {
      const rows = await request(`?scope=eq.${encodeURIComponent(scope)}&order=end_date.desc,generated_at.desc&limit=2000`);
      return { reports: rows.map(publicReport) };
    },
    async get(id) {
      const rows = await request(`?scope=eq.${encodeURIComponent(scope)}&id=eq.${encodeURIComponent(id)}&limit=1`);
    ...
    async upsertMany(inputs) {
      ...
      const byKey = new Map(inputs.map(input => {

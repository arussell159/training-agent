...
export async function updateLocalWorkout(id, change) {
  return mutateData((data) => {
    ...
    const approval = {
      ...
      updated_at,
      source: "coach-approval",
    };
    data.approved_changes = [
      approval,
      ...(data.approved_changes || []).filter((item) => item.id !== id),
    ];
    return approval;
  });
}

export async function addLocalComment(workoutId, body, type = "post") {
  return mutateData((data) => {
    const normalizedType = String(type || "post");
    const durableCoachNote = /^coach_note_(planned|completed)$/.test(normalizedType);
    const existingIndex = durableCoachNote
      ? data.comments.findIndex(
          (item) => item.workout_id === workoutId && item.type === normalizedType
        )
      : -1;
    const existing = existingIndex >= 0 ? data.comments[existingIndex] : null;
    const comment = {
      id:
        existing?.id ||
        (durableCoachNote ? `coach:${normalizedType}:${workoutId}` : `comment-${Date.now()}`),
      workout_id: workoutId,
      type: normalizedType,
      body,
      created_at: new Date().toISOString(),
    };
    if (existingIndex >= 0) data.comments.splice(existingIndex, 1);
...
export async function removePushSubscription(endpoint) {
  return mutateData((data) => {
    data.notification_preferences.subscriptions = (
      data.notification_preferences.subscriptions || []
...
export async function getDailyReviewByDate(athleteId, localDate) {
  const data = await readData();
  return (
    data.daily_reviews.find(
      (item) => item.athlete_id === athleteId && item.local_date === localDate
    ) || null
  );
}
...
export async function saveDailyReview(review) {
  return mutateData((data) => {
    const existingIndex = data.daily_reviews.findIndex(
      (item) =>
        item.id === review.id ||
        (item.athlete_id === review.athlete_id && item.local_date === review.local_date)
    );
    if (existingIndex >= 0) data.daily_reviews[existingIndex] = review;
    else data.daily_reviews.unshift(review);

    const existingConversation = data.coach_conversations.find(
      (item) => item.id === review.conversation_id || item.review_id === review.id
    );
    ...
    const conversation = normalizeConversation({
      ...
      title:
        new Date(`${review.local_date}T12:00:00Z`).toLocaleDateString("en-US", {
          month: "short",
          day: "2-digit",
          timeZone: "UTC",
        }) + " Review",
      ...
      messages: [
        {
          id: reviewMessageId,
          role: "assistant",
          content: review.conversation_text,
    ...
    const conversationIndex = data.coach_conversations.findIndex(
      (item) => item.id === conversation.id || item.review_id === review.id
    );
      ...
      conversation.messages = [
        {
          id: reviewMessageId,
          role: "assistant",
          content: next.conversation_text,

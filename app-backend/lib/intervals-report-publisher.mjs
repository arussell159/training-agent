  ...
  async function publish(target, text) {
    ...
    if (target.kind === "weekly") {
      const activities = await one(
        request,
        `/athlete/0/activities?oldest=${target.startDate}&newest=${target.endDate}`
      );
      const latest = activities
        .filter((activity) => activity?.id != null)
        .sort((a, b) =>
          String(b.start_date_local || b.date || b.created || "").localeCompare(
            String(a.start_date_local || a.date || a.created || "")
          )
        )[0];
      if (!latest)
        throw new Error("No completed workout is available for the weekly report comment.");

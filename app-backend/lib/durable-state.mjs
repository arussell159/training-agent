...
async function store() {
  ...
  const environment = Object.fromEntries(['SUPABASE_URL','SUPABASE_SECRET_KEY','SETTINGS_ENCRYPTION_KEY','SETTINGS_SCOPE'].filter(k => process.env[k]).map(k => [k,process.env[k]]));
  return createSupabaseSettingsStore(process.env.VERCEL ? {...local,...environment} : {...environment,...local});
}
// Files are migration inputs and read-through caches, never the durable save destination.
export function createDurableState({getStore=store,readFile=fs.readFile,writeFile=fs.writeFile}={}) {
return {
async read(name,cachePath,fallback) {
  ...
  catch {initial = typeof fallback === 'function' ? await fallback() : fallback;}
...
async write(name,value,cachePath) {
  ...
  if (cachePath) {try {await writeFile(cachePath,JSON.stringify(value,null,2));} catch { /* Cache failure cannot undo a confirmed database save. */ }}
},
};
}

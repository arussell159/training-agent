...
export function mergeHistoricalContext(context, archive, today) {
  ...
  const signature = w => `${w.workout_date}|${String(w.sport).toLowerCase()}|${String(w.title).trim().toLowerCase()}`;
  ...
  const historical = [...(archive.history || []), ...(archive.planned || [])]
    ...
    .map(w => ({...w,id:`tp-history:${w.id}`,source:'trainingpeaks-archive',read_only:true}));
  const history = [...(context.history || []), ...historical].sort((a,b) => a.workout_date.localeCompare(b.workout_date));

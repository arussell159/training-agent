...
export function createCoachReportCatalog({ store, legacySaved, legacyNotes, marker }) {
  let pending;
  const sourceKey = report => report.sourceKey || (
    report.kind === "weekly"
  ...
  async function migrate() {
    ...
    await store.upsertMany((notes.reports || []).filter(report => !existing.has(sourceKey(report))));
    await store.upsertMany((await legacySaved()).filter(report => !existing.has(sourceKey(report))));
    await marker.update(state => { state.complete = true; state.completedAt = new Date().toISOString(); });
  }
  return {
    async list() {
      ...
      try { await pending; } catch {
        ...
        const existing = new Set((await store.list()).reports.map(report => report.sourceKey));
        await store.upsertMany((await legacySaved().catch(() => [])).filter(report => !existing.has(sourceKey(report))));
      }
      return store.list();
    },
  };

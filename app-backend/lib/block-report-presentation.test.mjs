...
[[/SECTION11_REPORT:BLOCK:2026-08-10]]`);
  ...
  assert.deepEqual(section("Durability").rows[1], ["Wk 4", "no qualifying durability session", "—"]);
  assert.match(section("Compliance").prose[0], /29.67 h; history.json: 30.37 h/);
  assert.deepEqual(section("Phase Progression Check").prose, ["Recommendation: progress with caution."]);
...
test("no summary or numerical values are fabricated for missing sections", () => {
  assert.deepEqual(parseBlockReportPresentation("COMPLIANCE\nNo data retained."), {
    summary: [], sections: [{ title: "Compliance", headers: ["Metric", "Result"], rows: [], prose: ["No data retained."] }],
  });
});

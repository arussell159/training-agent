...
test("app instructions use discipline-specific human-readable formatting", () => {
  const swim = racePlan.find((p) => p.name === "Swim 12x100 CSS").description;
  assert.match(
    swim,
  ...
  const bike = racePlan.find((p) => p.name === "Bike Race Rehearsal").description;
  assert.match(bike, /Warm Up:\n15 mins at 112 → 132 W\./);
  assert.match(bike, /Main Set:\n4 x \(8 mins at 154 W \+ 2 mins at 146 W\),/);
  const run = racePlan.find((p) => p.name === "Run 3x6min Threshold").description;
  assert.match(run, /Warm Up:\n15 mins easy jog at 10:30–11:00 min\/mile\./);
  assert.match(
    run,
    /3 x \(6 mins at 8:50–9:00 min\/mile \+ 2 mins recovery at 10:30–11:00 min\/mile\),/
  );
});
test("swim work uses yards converted to meters, zone targets, and seconds-only rests", () => {
  for (const p of racePlan.filter((p) => p.type === "Swim"))
    for (const s of leaves(p.workout_doc.steps)) {
      if (s.intensity === "rest") {
        assert.ok(s.duration > 0);
        assert.equal(s.distance, undefined);
        assert.equal(s.pace, undefined);
      } else {
        assert.ok(Math.abs(s.distance / 0.9144 - Math.round(s.distance / 0.9144)) < 1e-8);
        assert.equal(s.pace.units, "pace_zone");
        assert.ok(Number.isInteger(s.pace.value) && s.pace.value >= 1 && s.pace.value <= 5);
      }
    }
  assert.equal(
    Math.round(racePlan.find((p) => p.name === "Swim 12x100 CSS").workout_doc.distance / 0.9144),
    2200
  );
});
test("Garmin swim definitions use yard amounts with the Intervals mtr parser workaround and explicit yard paces", () => {
  ...
  const css = nativeWorkoutDefinition(racePlan.find((p) => p.name === "Swim 12x100 CSS"));
...
test("run and bike targets are absolute athlete-specific values, with accurate total durations", () => {
  for (const p of racePlan.filter((p) => p.type !== "Swim"))
    for (const s of leaves(p.workout_doc.steps)) {
      ...
      assert.equal(s.distance, undefined);
      assert.equal(
        p.type === "Run" ? s.pace.units : s.power.units,
        p.type === "Run" ? "secs/mi" : "w"
      );
    }
  assert.equal(
    racePlan.find((p) => p.name === "Bike 3x5min Race Power").workout_doc.duration,
    75 * 60
  );
  assert.equal(
    racePlan.find((p) => p.name === "Bike Race Rehearsal").workout_doc.duration,
    180 * 60
  );
  assert.match(
    nativeWorkoutDefinition(racePlan.find((p) => p.name === "Bike 3x5min Race Power")),
    /3x\n- 30s 173w\n- 30s 106w/
  );
  assert.match(
    nativeWorkoutDefinition(racePlan.find((p) => p.name === "Run 3x6min Threshold")),

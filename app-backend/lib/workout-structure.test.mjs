...
import {hasWorkoutStructure,structuredWorkoutProfile,workoutProfileSegments,workoutStepLabel} from '../../ui/src/lib/workout-structure.ts';
...
test('hover metadata retains actual repetition groups and sport-specific target units',()=>{
 const work={text:'Steady',distance:400*.9144,duration:400,pace:{value:3,units:'pace_zone'}};
 const rest={duration:30,intensity:'rest'};
 const segments=workoutProfileSegments(JSON.stringify({steps:[{reps:3,steps:[work,rest]}]}));
...
test('actual repeat durations, recoveries, ramps and targets drive the profile',()=>{
 const value=JSON.stringify({steps:[{duration:60,power:{value:100}},{reps:2,steps:[{duration:120,power:{value:200}},{duration:30,intensity:'rest'}]},{duration:60,ramp:true,power:{start:100,end:200}}]});

...
test("athlete-confirmed historical completions are green; future and uncompleted workouts are untouched", () => {
  const history = [
    { id: "old", status: "completed", workout_date: "2026-09-14" },
    { id: "future", status: "completed", workout_date: "2026-09-16" },
    { id: "planned", status: "today", workout_date: "2026-09-15" },
  ];
  const result = applyCompletionConfirmation(
  ...
  assert.equal(result.history[1].completion_grade, undefined);
  assert.equal(result.history[2].completion_grade, undefined);

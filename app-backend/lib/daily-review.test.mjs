...
test("uses the athlete time zone and schedules one hour before an early workout", () => {
  assert.deepEqual(localClock(new Date("2026-09-14T11:30:00Z"), "America/Chicago"), {
    date: "2026-09-14",
    time: "06:30",
  });
  assert.equal(
    effectiveReviewTime(
      [{ ...workout, scheduled_start_at: "2026-09-14T12:00:00Z" }],
      "08:00",
      "America/Chicago"
    ),
    "06:00"
...
test("skips rest and completed sessions", () => {
  const selected = scheduledWorkouts(
    {
      planned: [
        workout,
        { ...workout, id: "rest", sport: "Recovery" },
...
test("a normal day is proceed as planned and needs no approval", () => {
  ...
  assert.ok(
    review.workouts[0].execution_guidance.target_ranges.every((range) =>
      /no individualized numerical adjustment/i.test(range)
    )
  );
  assert.doesNotMatch(review.conversation_text, /Stop (?:the )?main set|Stop rule/i);
});

test("conversation copy changes hierarchy without dropping existing review content", () => {
  const review = createDailyReview({
    athleteId: "a1",
    localDate: "2026-09-14",
    timeZone: "America/Chicago",
    context,
    workouts: [workout],
    decision: heuristicDecision(context, [workout], "2026-09-14"),
  });
  const text = reviewConversationText(review);
  assert.match(text, /^\*\*Today’s recommendation: Go\*\*/);
  assert.match(text, /## Bike – 3x8min/);
  assert.match(text, /## Athlete metrics/);
  assert.match(text, /\*\*Fitness:\*\*/);
});
...
test("poor sleep supports a small reduction without changing zones", () => {
  const poorSleep = {
    ...context,
    history: [{ workout_date: "2026-09-14", recovery: { sleep_hours: 4.8 } }],
  };
...
test("repeated failed comparable sessions reduce the target, while one does not redefine fitness", () => {
  const failedHistory = [1, 2].map((day) => ({
    workout_date: `2026-09-${10 + day}`,
    sport: "Bike",
    completed: { duration_minutes: 45, rpe: 9 },
    compliance: 65,
    failure_signals: ["late_interval_fade"],
  }));
  const decision = heuristicDecision(
    { ...context, history: failedHistory },
    [workout],
    "2026-09-14"
  );
  assert.equal(decision.workouts[0].action, "reduce_target");
...
test("suspected bonking does not automatically change the workout", () => {
  const bonkContext = {
    ...context,
    comments: [{ body: "I bonked late after missing breakfast." }],
  };
...
test("a taper does not increase training after a few easy sessions", () => {
  const easyHistory = [10, 11, 12].map((day) => ({
    workout_date: `2026-09-${day}`,
    sport: "Bike",
    completed: { duration_minutes: 60, rpe: 3 },
    compliance: 100,
    failure_signals: [],
  }));
  const decision = heuristicDecision(
    { ...context, athlete: { phase: "taper" }, history: easyHistory },
    [workout],
    "2026-09-14"
  );
  assert.equal(decision.workouts[0].action, "follow_as_written");
...
test("calendar changes invalidate a pending recommendation", () => {
  ...
  const review = createDailyReview({
    ...
    localDate: "2026-09-14",
    timeZone: "America/Chicago",
    context: { ...context, metrics: { recovery: 30 } },
    workouts: [workout],
    decision,
  });
  assert.equal(
    materiallyChanged(review, { ...context, metrics: { recovery: 30 } }, [
      { ...workout, plannedDurationMinutes: 75 },
    ]),
    true
  );
});
...
test("multiple same-day sessions are combined into one review", () => {
  ...
  const review = createDailyReview({
    ...
    localDate: "2026-09-14",
    timeZone: "America/Chicago",
    context: { ...context, planned: workouts },
    workouts,
    decision,
  });
...
test("recovery allowances use one consistent wording pattern", () => {
  ...
  for (const range of [fifties, hundreds]) {
    assert.match(
      range,
      /maintain the target and increase rest by no more than 10 seconds if needed\.$/i
    );
    assert.doesNotMatch(range, /add no more than|allow up to/i);
  }
  assert.match(
    fourHundreds,
    /keep the written recovery and allow up to 5 sec\/100yd slower if needed\.$/i
  );

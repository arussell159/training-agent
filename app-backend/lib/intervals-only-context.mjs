...
export function intervalsOnlyContext(context) {
  const keep = (w) => {
    ...
    if (!/^(activity|event):/.test(id) || w.read_only || w.source === "trainingpeaks-archive")
      return false;
    if (
      /^\d{4}-\d{2}-\d{2}$/.test(String(w.workout_date || "")) &&
      w.workout_date >= FIT_CLEANUP_BEFORE
    )
      return true;
    return (
      Boolean(w.activity_id || id.startsWith("activity:")) &&
      String(fileType || "").toLowerCase() === "fit"
    );
  };
  return {
    ...context,
    history: (context.history || []).filter(keep),
    planned: (context.planned || []).filter(keep),
    workouts: (context.workouts || context.history || []).filter(keep),
  };
}

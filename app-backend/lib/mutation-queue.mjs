...
export function pendingMutationContext(context, mutation) {
  ...
  const map = (workout) => {
    ...
    return {
      ...
      ...patch,
      sync_status: "pending",
      sync_operation: mutation.operationId,
...
export function createMutationQueue(config, store) {
  ...
  const write = (id, cursor) =>
    store.upsert("sync_state", [
      { athlete_id: prefix + id, status: "mutation", cursor, updated_at: new Date().toISOString() },
    ]);
  return {
    async enqueue(input) {
      ...
      if (previous) {
        if (JSON.stringify(previous.mutation) !== JSON.stringify(mutation))
      ...
      for (const row of older)
        if (row.cursor.mutation.id === mutation.id && row.cursor.mutation.type === mutation.type)
          await write(row.cursor.mutation.operationId, { ...row.cursor, state: "superseded" });
      return job;
    },
    async drain(apply, { retryFailed = false, now = Date.now() } = {}) {
      const rows = (await store.listSyncRecords(prefix)).sort((a, b) =>
        a.cursor.created_at.localeCompare(b.cursor.created_at)
      );
      let synced = 0,
        failed = 0,
        pending = 0;
      ...
      for (const row of rows) {
        ...
        if (blocked.has(job.mutation.id)) {
          pending++;
          continue;
        }
        if (job.state === "failed" && !retryFailed) {
          failed++;
          blocked.add(job.mutation.id);
          continue;
        }
        if (job.next_retry_at > now && !retryFailed) {
          pending++;
          blocked.add(job.mutation.id);
          continue;
        }
        try {
          ...
          await write(job.mutation.operationId, {
            ...job,
            state: "synced",
            error: null,
          ...
          await write(job.mutation.operationId, {
            ...
            state,
            error: error.message,
            next_retry_at: now + Math.min(1800000, 60000 * 2 ** attempts),
          });

...
import { assertRecommendationEvidence, athleteEvidence } from "./recommendation-evidence.mjs";
...
function assertWorkoutPatches(decision, workouts) {
  ...
  for (const item of decision.workouts || []) {
    ...
    if (!workout) {
      errors.push(`unknown workout ${item.workout_id}`);
      continue;
    }
  ...
  if (errors.length)
    throw new Error(`Recommendation workout validation failed: ${errors.join("; ")}`);
}

async function verifiedFollowDecision(context, workouts, localDate) {
  return evidenceInsufficientDecision(
    context,
    workouts,
    localDate,
    "The coaching model is unavailable. No Section 11 decision was made; retry when the connection is restored."
  );
}
...
const REVIEW_INSTRUCTIONS = `Perform a pre-workout daily endurance coaching review using Section 11 and the supplied dated athlete context. Follow Section 11's source and decision hierarchy when feedback, performance, metrics, and the written plan conflict. Determine the phase only from current verified data. State the purpose of each session and the reasoning for the decision. Treat protocol examples as illustrations, never default prescriptions.
...
Every recommendation requires dated athlete facts, explicit reasoning, coaching_judgment, applicability limits, and server-verifiable calculations for personalized numerical adjustments. No separate legacy evidence library is supplied: leave published empty and use Section 11 as the governing protocol. Preserve the athlete's recorded zone system.
Choose the supported action that best fits Section 11 and the athlete context. Guidance must describe the actual main-set groups and use verified targets, distance, duration, and recovery with explicit units. Do not restrict decisions to a predetermined recovery-versus-target lever.
...
Use the guide and current athlete context to decide; do not force a legacy metric-based verdict.`,
  ...
  if (!response.ok)
    throw new Error(
      `OpenAI daily review failed (${response.status}): ${(await response.text()).slice(0, 300)}`
    );
  ...
  assertRecommendationEvidence(
    decision.workouts.map((item) => ({
      action: item.proposed_change || "Follow the scheduled workout as written.",
      evidence: item.evidence,
    })),
    { passages: retrievedSources, athleteFacts }
  );
...
async function requestStructuredRefinement(config, review, context, workouts, instruction) {
  if (!config.OPENAI_API_KEY)
    throw new Error("OpenAI is not configured for recommendation refinements");
  const coachingInstructions = await loadCoachingInstructions();
  const retrievedSources = [];
...
Revise the unresolved saved review using the athlete's latest reply. Treat that reply as a requested constraint, not approval. Change only the workout, interval group, targets, recovery, or fields explicitly named by the athlete. Preserve every unmentioned workout and main-set group exactly. The patch must contain the complete Intervals.icu-ready replacement needed to apply the displayed revision, while leaving unmentioned intervals unchanged. Describe the revised workout as a suggestion awaiting approval.`,
      ...
      text: {
        format: {
          type: "json_schema",
          name: "daily_workout_review_refinement",
          strict: true,
          schema: dailyReviewSchema,
        },
        verbosity: "low",
  ...
  if (!response.ok)
    throw new Error(
      `OpenAI review refinement failed (${response.status}): ${(await response.text()).slice(0, 300)}`
    );
  const text = outputText(await response.json());
...
function applyPatchToSnapshot(snapshot, patch) {
  return {
    ...
    ...(Object.hasOwn(patch, "totalTimePlanned")
      ? { duration_minutes: Math.round(Number(patch.totalTimePlanned) * 60) }
      : {}),
...
async function requestConditionReview(config, context, workouts, localDate) {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      reasoning: { effort: "low" },
      instructions: `${policy}\n\nDaily review delivery: return the required JSON with a brief condition assessment and up to three suggestions. This scheduled review is advisory only and does not modify workouts. Analyse the supplied current Intervals.icu snapshot using Section 11.`,
      input: JSON.stringify({
        ...
        recent_history: (context.history || context.workouts || []).slice(-90),
        today: workouts,
        upcoming: context.planned,
      }).slice(0, 100000),
      text: {
        format: {
          type: "json_schema",
          name: "daily_condition_review",
          strict: true,
          schema: {
            type: "object",
            ...
            required: ["summary", "condition", "suggestions", "uncertainty"],
          },
        },
      },
    }),
...
export function createDailyReviewService({
  ...
  async function generate(context, workouts, preferences, previous = null, now = new Date()) {
    ...
    try {
      ...
      decision = advisoryOnly
        ? await requestConditionReview(config, context, workouts, localDate)
        : await decisionProvider(config, context, workouts, localDate);
    } catch (error) {
      log(`daily review model fallback: ${error.message}`);
      try {
        decision = await verifiedFollowDecision(context, workouts, localDate);
      } catch (fallbackError) {
        decision = evidenceInsufficientDecision(
          context,
          workouts,
          localDate,
          `The evidence chain could not be validated: ${error.message}; fallback failed: ${fallbackError.message}`
        );
      }
    ...
    if (advisoryOnly) {
      review.advisory = decision.advisory || {
        condition: decision.reason,
        suggestions: [],
        uncertainty:
          "The coaching model is unavailable; this review cannot assess your condition yet.",
      };
      ...
      review.conversation_text = [
        review.summary,
        review.advisory.condition,
        ...review.advisory.suggestions.map((item) => `- ${item}`),
  ...
  async function deliver(review) {
    ...
    if (!preferences.enabled) {
      return updateDailyReview(review.id, (current) => ({
        ...current,
        notification: { ...current.notification, state: "disabled", last_error: null },
      }));
    }
    const subscriptions = preferences.subscriptions || [];
    if (!subscriptions.length) {
      await updateNotificationPreferences({
        last_delivery_error: "Notifications are enabled, but this device is not subscribed.",
      });
      return updateDailyReview(review.id, (current) => ({
        ...current,
        notification: {
          ...
          state: "unavailable",
          last_error: "No push subscription is registered.",
        },
      }));
    }
    ...
    webpush.setVapidDetails(
      config.VAPID_SUBJECT || "mailto:coach@arperformance.local",
    ...
    const payload = JSON.stringify({
      ...
      body:
        ...
        (review.changes_proposed ? "Workout adjustment suggested." : "Proceed as planned."),
    ...
    await updateNotificationPreferences({
      last_delivery_error: lastError || "Push delivery failed.",
    });
    return updateDailyReview(review.id, (current) => ({
      ...current,
      notification: {
    ...
    if (!advisoryOnly && !workouts.length) {
      return updateDailyReview(review.id, (current) => ({
        ...
        notification: {
          ...current.notification,
          state: "cancelled",
          last_error: "No scheduled workout remained when the calendar was rechecked.",
        },
      }));
    }
  ...
  async function runDue(now = new Date(), { force = false, refresh = false } = {}) {
    ...
    try {
      ...
      if (
        (!advisoryOnly && !workouts.length) ||
        (!force &&
          !isReviewDue({
            now,
            timeZone: preferences.time_zone,
            configuredTime: preferences.review_time,
            workouts,
          }))
      )
        return null;
      ...
      const needsEvidenceUpgrade = Boolean(
        review && (advisoryOnly ? !review.advisory : review.evidence_status !== "verified")
      );
      const policyChanged = Boolean(
        review && review.snapshot?.coaching_policy_version !== (context.coaching?.version || null)
      );
      if (!review || (refresh && (snapshotChanged || needsEvidenceUpgrade || policyChanged))) {
        if (
          refresh &&
          review &&
          ["approved", "denied", "cancelled", "expired"].includes(review.status)
        )
          return review;
      ...
      if (
        ["approved", "denied", "cancelled", "expired", "evidence_insufficient"].includes(
          review.status
        )
      )
        return review;
      ...
      if (
        preferences.enabled &&
        !(preferences.subscriptions || []).length &&
        review.notification?.state === "unavailable"
      )
        return review;
  ...
  async function refine(id, instruction, now = new Date()) {
    if (advisoryOnly)
      return {
        ...
        body: {
          error:
            "Daily reviews are advice only. Discuss adjustments in chat; workouts will not be edited.",
        },
      };
    ...
    if (!["pending_approval", "proceed_as_planned", "apply_failed"].includes(review.status)) {
      return {
        status: 409,
        body: { error: "This daily review can no longer be revised.", review },
      };
    }
    if (localClock(now, review.time_zone).date !== review.local_date) {
      review = await updateDailyReview(id, (current) => ({
        ...current,
        status: "expired",
        decision: { type: "expired", decided_at: now.toISOString() },
      }));
      return {
        status: 409,
        body: { error: "This recommendation expired when the workout day ended.", review },
      };
    }
    const config = await readConfig();
    const latest = await getContext(config, { force: true, strict: true });
    ...
    if (
      ...
      reviewedWorkouts.some((workout) => workoutHasStarted(workout, now)) ||
      reviewedWorkouts.length !== reviewedIds.size
    ) {
      return {
        status: 409,
        body: {
          error:
            "A reviewed workout has started, completed, moved, or been removed. Nothing was revised.",
          review,
        },
      };
    }
  ...
  async function approve(id, now = new Date()) {
    if (advisoryOnly)
      return {
        status: 409,
        body: { error: "Daily reviews are advice only and cannot edit workouts." },
      };
    let review = await getDailyReview(id);
    if (!review) return { status: 404, body: { error: "Daily review not found" } };
    ...
    if (review.status === "denied")
      return {
        status: 409,
        body: { error: "This proposal was denied and cannot be applied.", review },
      };
    if (review.evidence_status !== "verified")
      return {
        status: 409,
        body: {
          error: "This review has no validated evidence chain, so no change can be applied.",
          review,
        },
      };
    if (!review.changes_proposed)
      return { status: 409, body: { error: "This review has no proposed changes.", review } };
    if (localClock(now, review.time_zone).date !== review.local_date) {
      review = await updateDailyReview(id, (current) => ({
        ...current,
        status: "expired",
    ...
    if (!config.INTERVALS_API_KEY)
      return {
        status: 503,
        body: { error: "Intervals.icu is not connected, so no change was applied.", review },
      };
    const latest = await getContext(config, { force: true, strict: true });
    const workouts = scheduledWorkouts(latest, review.local_date);
    const currentById = new Map(workouts.map((item) => [String(item.id), item]));
    if (
      review.workouts.some(
        (item) =>
          !currentById.has(item.workout_id) ||
          workoutHasStarted(currentById.get(item.workout_id), now)
      )
    ) {
      review = await updateDailyReview(id, (current) => ({
        ...current,
      ...
      return {
        ...
        body: {
          error:
            "A reviewed workout has started, completed, moved, or been removed. Nothing was changed.",
          review,
        },
      };
    }
    if (materiallyChanged(review, latest, workouts)) {
      ...
      return {
        ...
        body: {
          error:
            "The workout, schedule, or recovery context changed. The proposal was refreshed for approval; nothing was applied.",
          review,
          refreshed: true,
        },
      };
    ...
    if (claimed.operation_id !== operationId)
      return {
        status: 409,
        body: { error: "This review is already being resolved.", review: claimed },
      };

    try {
      for (const item of claimed.workouts) {
        if (
          item.action === "follow_as_written" ||
          !Object.keys(item.patch || {}).length ||
          item.applied_at
        )
          continue;
        const result = await applyWorkoutPatch(config, item.workout_id, item.patch, {
          now: new Date(),
          timeZone: claimed.time_zone,
        });
        review = await updateDailyReview(id, (current) => {
          const snapshots = current.snapshot.workouts.map((snapshot) =>
            snapshot.id === item.workout_id ? applyPatchToSnapshot(snapshot, item.patch) : snapshot
          );
          return {
            ...current,
            snapshot: {
            ...
            workouts: current.workouts.map((workout) =>
              workout.workout_id === item.workout_id
                ? { ...workout, applied_at: new Date().toISOString(), verification: result }
                : workout
            ),
          };
        });
      ...
      review = await updateDailyReview(id, (current) => ({
        ...
        status: "approved",
        operation_id: null,
        decision: { type: "approved", decided_at: new Date().toISOString() },
      }));
      ...
      return {
        status: 502,
        body: { error: `Intervals.icu did not confirm every change: ${error.message}`, review },
      };
    }
  }
  ...
  async function deny(id, now = new Date()) {
    const review = await updateDailyReview(id, (current) => {
      ...
      return {
        ...current,
        status: "denied",
        decision: { type: "denied", decided_at: now.toISOString() },
      };
    });
    if (!review) return { status: 404, body: { error: "Daily review not found" } };
    if (review.status !== "denied")
      return { status: 409, body: { error: "This review can no longer be denied.", review } };
    return { status: 200, body: { review } };
  }
  ...
  return {
    ...
    async updatePreferences(input) {
      ...
      if (/^([01]\d|2[0-3]):[0-5]\d$/.test(input.reviewTime || ""))
        patch.review_time = input.reviewTime;

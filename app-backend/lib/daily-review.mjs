            ...
            properties: {
              published: {
                ...
                items: {
                  type: "object",
                  additionalProperties: false,
                  required: ["source_id", "passage_id", "claim"],
                  properties: {
              ...
              calculations: {
                ...
                items: {
                  type: "object",
                  additionalProperties: false,
                  required: ["id", "operation", "inputs", "result", "unit"],
                  properties: {
                    id: { type: "string" },
                    operation: {
                      type: "string",
                      enum: ["difference", "sum", "product", "percent_of", "percent_change"],
                    },
                    inputs: {
                      type: "array",
                      minItems: 1,
...
function minutes(value) {
  ...
  return Math.max(
    0,
    Math.min(1439, (Number.isFinite(hour) ? hour : 0) * 60 + (Number.isFinite(minute) ? minute : 0))
  );
}
...
export function effectiveReviewTime(workouts, configuredTime, timeZone) {
  ...
  const starts = workouts
    .map((workout) => plannedStartClock(workout, timeZone))
    .filter(Boolean)
    .map(minutes);
...
export function scheduledWorkouts(context, localDate) {
  return (context.planned || []).filter((workout) => {
    ...
    return (
      workout.workout_date === localDate &&
      workout.status !== "completed" &&
      workout.sport !== "Recovery" &&
      duration > 0
    );
  });
}
...
function evidenceFor(context, workout, localDate) {
  ...
  const comparable = recent
    .filter(
      (item) =>
        String(item.sport || "").toLowerCase() === String(workout.sport || "").toLowerCase() &&
        Number(item.completed?.duration_minutes ?? item.actualDurationMinutes ?? 0) > 0
    )
    .slice(-8)
    .reverse();
...
function allRecentText(context, localDate) {
  ...
  return [
    ...(context.comments || [])
      .filter((item) => !item.created_at || String(item.created_at).slice(0, 10) >= cutoffDate)
      .slice(0, 10)
      .map((item) => item.body),
    ...(context.history || [])
      .filter((item) => item.workout_date >= cutoffDate)
      .slice(-10)
      .flatMap((item) => [item.post_comment, item.athleteComments]),
  ]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
...
function athleteMetrics(context, localDate) {
  ...
  const recentRecovery =
    snapshot.latest.find(
      (item) => item.recovery && Object.values(item.recovery).some((value) => value != null)
    )?.recovery || {};
  const historyRecovery = (context.history || [])
    .filter((item) => item.workout_date <= localDate)
    .slice(-90)
  ...
  const withBaseline = (value, baseline, unit) => {
    ...
    if (!Number.isFinite(baseline) || baseline <= 0)
      return `${Math.round(value)} ${unit}; no personal baseline is available.`;
    const difference = Math.round(value - baseline);
    return `${Math.round(value)} ${unit}; ${Math.abs(difference)} ${unit} ${difference === 0 ? "from" : difference > 0 ? "above" : "below"} the 90-day median of ${Math.round(baseline)} ${unit}.`.replace(
      "0 " + unit + " from",
      "in line with"
    );
  };
  return {
    fitness: Number.isFinite(fitness)
      ? `${Math.round(fitness)} (training-load estimate)`
      : "Unavailable",
    fatigue: Number.isFinite(fatigue)
      ? `${Math.round(fatigue)} (training-load estimate)`
      : "Unavailable",
    form: Number.isFinite(form) ? `${Math.round(form)} (training-load estimate)` : "Unavailable",
    recovery:
      Number.isFinite(recovery) && recovery > 0 ? `${Math.round(recovery)}%` : "Unavailable",
    hrv: withBaseline(hrv, hrvBaseline, "ms") || "No current HRV reading is available.",
    resting_heart_rate:
      withBaseline(restingHeartRate, restingHeartRateBaseline, "bpm") ||
      "No current resting-heart-rate reading is available.",
  };
}

function mainSetLines(workout) {
  const lines = String(workout.details || workout.goal || "")
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  const mainIndex = lines.findIndex((line) => /^main set:/i.test(line));
  const warmDownIndex = lines.findIndex(
    (line, index) => index > mainIndex && /^warm down:|^cool down:/i.test(line)
  );
  const source =
    mainIndex >= 0
...
function baseGuidance(action, workout) {
  ...
  if (action === "substitute_easy")
    return {
      target_ranges: [
        "Easy replacement at RPE 2–3/10: keep it fully conversational with no interval targets.",
      ],
    };
  const sport = String(workout.sport || "").toLowerCase();
  if (action === "follow_as_written")
    return {
      target_ranges: mainSetLines(workout).map(
        (line) =>
          `${line}: follow the scheduled target and recovery; no individualized numerical adjustment is being added.`
      ),
    };
  const actionLead =
  ...
  return {
    target_ranges: mainSetLines(workout).map((line) => {
      ...
      if (sport === "swim") {
        if (quality)
          return `${line}: ${actionLead} Maintain the prescribed pace or zone; maintain the target and increase rest by no more than 30 seconds if needed.`;
      ...
      if (sport === "bike")
        return `${line}: ${actionLead} Keep the written recovery; power may be reduced by up to 5% if needed to preserve the intended effort.`;
      if (sport === "run")
        return `${line}: ${actionLead} Keep the written recovery; pace may be up to 10 sec/mi slower if needed to preserve the intended effort.`;
...
export function heuristicDecision(context, workouts, localDate) {
  ...
  const concerning =
    /chest pain|faint|fainted|confusion|severe unusual breath|fever|worsening pain/.test(
      recentText
    );
  const todayRecovery = recoverySnapshot(context, localDate).latest[0]?.recovery || {};
  const lowRecovery =
    Number(context.metrics?.recovery) > 0 && Number(context.metrics.recovery) < 45;
  const poorSleep =
    Number(todayRecovery.sleep_hours) > 0 && Number(todayRecovery.sleep_hours) < 5.5;
  const relevantObservations = [];
  const decisions = workouts.map((workout) => {
    const evidence = evidenceFor(context, workout, localDate);
    const failed = evidence.comparable.filter(
      (item) => (item.failure_signals || []).length || Number(item.compliance) < 75
    );
    const easy = evidence.comparable.filter(
      (item) => Number(item.completed?.rpe) <= 4 && Number(item.compliance) >= 95
    );
    let action = "follow_as_written";
    let reason =
      "Recent execution and available recovery do not show a consistent reason to change today’s prescription.";
    ...
    if (concerning) {
      action = "rest";
      reason =
        "Recent feedback includes a concerning symptom; health takes priority over the planned stimulus.";
      proposedChange =
        "Replace this workout with rest and seek appropriate assessment if the symptom is current or persistent.";
      patch = {
        ...patch,
        title: `${workout.sport} – Rest`,
        description:
          "Rest. Do not start the planned session while concerning symptoms are present.",
        coachComments:
          "Daily review: rest recommended because recent feedback included a concerning symptom.",
        totalTimePlanned: 0,
        tssPlanned: 0,
      };
      ...
      reason =
        failed.length >= 2
          ? "Multiple recent comparable sessions show incomplete execution, so a small reduction protects repeatable quality without redefining training zones."
          : "Available recovery information is meaningfully below normal, so reducing today’s output should preserve the session’s purpose.";
      proposedChange =
        "Keep the session structure but reduce prescribed pace or power by about 3–5%; do not add volume.";
      patch = {
        ...patch,
        coachComments:
          `Daily review: use 95–97% of the written pace or power target today and preserve the intended effort. ${workout.goal || ""}`.trim(),
      };
    } else if (failed.length === 1) {
      action = "add_recovery";
      reason =
        "One recent comparable session was difficult, which supports protecting interval quality but not changing zones or removing the stimulus.";
      proposedChange =
        "Keep the work target and allow up to 60 seconds extra recovery on no more than two recoveries.";
      patch = {
        ...patch,
        coachComments:
          `Daily review: prioritize interval quality. Up to 60 seconds extra recovery is allowed twice. ${workout.goal || ""}`.trim(),
      };
    } else if (easy.length >= 3 && context.athlete?.phase !== "taper") {
      action = "increase_modestly";
      reason =
        "At least three recent comparable sessions were completed comfortably with good adherence and no conflicting recovery signal.";
      proposedChange =
        "Increase the main-set target by about 2–3%, without adding repetitions or duration.";
      patch = {
        ...patch,
        coachComments:
          `Daily review: a modest 2–3% main-set target increase is supported. Do not add repetitions or duration. ${workout.goal || ""}`.trim(),
      };
    }
    if (failed[0]) {
      relevantObservations.push(
        `${failed[0].workout_date}: ${failed[0].title || `${workout.sport} workout`} was not completed as intended${Number.isFinite(Number(failed[0].compliance)) ? ` (${Math.round(Number(failed[0].compliance))}% completion)` : ""}.`
      );
    } else if (easy[0]) {
      relevantObservations.push(
        `${easy[0].workout_date}: ${easy[0].title || `${workout.sport} workout`} was completed comfortably at RPE ${Number(easy[0].completed?.rpe)}/10 with ${Math.round(Number(easy[0].compliance))}% completion.`
      );
    }
    const priority = /threshold|vo2|race|interval|brick/i.test(`${workout.title} ${workout.goal}`)
      ? "key"
      : "supporting";
    const targetFlexibility =
      priority === "key"
        ? action === "follow_as_written"
          ? "This is the key session today, so protect smooth, repeatable interval quality; the targets are guides, not pass/fail numbers."
          : "This is the key session today, so use only the displayed adjustment and protect repeatable interval quality."
        : action === "follow_as_written"
          ? "This session supports the week, so keep it controlled and let good technique and the intended effort matter more than an exact number."
          : "This session supports the week, so keep the adjustment conservative and finish without adding extra load.";
    return {
  ...
  return {
    summary: changed
      ? "A small adjustment is recommended for today’s training."
      : "Proceed as planned",
    notification_summary: changed ? "Workout adjustment suggested." : "Proceed as planned.",
    reason: changed
      ? "The recommendation uses the smallest change supported by recent comparable execution, feedback, recovery, and today’s session purpose."
      : "Today’s sessions match the available recovery and recent execution evidence.",
...
export function evidenceInsufficientDecision(
  context,
  workouts,
  localDate,
  reason = "No fully validated athlete-data and coaching-reasoning chain was available."
) {
  return {
    ...
    workouts: workouts.map((workout) => ({
      ...
      priority: "supporting",
      target_flexibility:
        "No individualized execution guidance is presented until its evidence is validated.",
      action: "follow_as_written",
      proposed_change: null,
      reason:
        "The coach is withholding a prescription; this does not establish that the current state is normal.",
...
export function lockDecisionToVerdict(generated, baseline) {
  ...
  const workouts = baseline.workouts.map((locked) => {
    ...
    const candidatePatch =
      candidate.patch && typeof candidate.patch === "object" ? candidate.patch : null;
    const candidateHasChange = candidatePatch
      ? Object.values(candidatePatch).some(
        ...
        action === "follow_as_written"
          ...
          : String(
              candidate.proposed_change ||
                locked.proposed_change ||
                "Apply the displayed coaching adjustment."
            ),
...
function cleanPatch(patch, original) {
  ...
  for (const field of ["totalTimePlanned", "tssPlanned"]) {
    if (Number.isFinite(source[field]) && Number(source[field]) >= 0)
...
export function validateWorkoutRecommendation(item, workout) {
  const errors = [];
  if (String(item?.workout_id) !== String(workout?.id))
    errors.push("workout id does not match the reviewed athlete workout");
  const patch = item?.patch || {};
  if (patch.structure) {
    let structure;
    try {
      structure = JSON.parse(patch.structure);
    } catch {
      errors.push("replacement structure is not valid JSON");
    }
    if (structure) {
      const steps = nestedWorkoutSteps(structure);
      if (String(workout.sport).toLowerCase() === "swim") {
        if (structure.visualizationDistanceUnit !== "yard")
          errors.push("swim structure must preserve visualizationDistanceUnit=yard");
        for (const step of steps) {
          const rest = step.intensityClass === "rest" || /\brest\b/i.test(step.name || "");
          if (!rest && step.length?.unit === "second")
            errors.push("swimming work was incorrectly encoded as seconds");
          if (rest && step.length?.unit !== "second")
            errors.push("passive swim rest must be encoded in seconds");
        }
      }
      const endSeconds = Math.max(
        0,
        ...(structure.structure || []).map((group) => Number(group.end)).filter(Number.isFinite)
      );
      if (
        Number.isFinite(patch.totalTimePlanned) &&
        endSeconds > 0 &&
        Math.abs(Number(patch.totalTimePlanned) * 3600 - endSeconds) > 60
      )
        errors.push("planned workout total does not match the replacement structure");
    }
  }
...
function notificationSummary(workouts, changesProposed) {
  ...
  const actions = new Set(
    workouts.filter((item) => item.action !== "follow_as_written").map((item) => item.action)
  );
  if ([...actions].every((action) => action === "add_recovery")) return "Extra recovery suggested.";
  if ([...actions].every((action) => action === "increase_modestly"))
    return "Higher targets suggested.";
  if (
    [...actions].every((action) =>
      ["reduce_target", "shorten", "remove_repetitions", "substitute_easy", "rest"].includes(action)
    )
  )
    return "Easier targets suggested.";
  const sports = new Set(
    workouts
      .filter((item) => item.action !== "follow_as_written")
      .map((item) => item.sport)
      .filter(Boolean)
  );
...
function ensureAdjustment(range, workout) {
  let value = String(range)
    .replace(/,\s*\((build|steady|strong)\),/i, ",")
    .replace(/[.;\s]+$/, "");
  const targetIsPrimary =
    /\b(?:build|strong|css|threshold|vo2|race[ -]?(?:pace|power|effort)|target pace|target power|z4|z5)\b/i.test(
      value
    );
  const stripTargetReduction = (input) =>
    input
      .replace(
        /;?\s*(?:keep (?:the )?written (?:rest|recovery)(?: unchanged)?(?: and)?\s*)?(?:you may\s+)?(?:allow(?: up to)?\s+)?\d+(?:\.\d+)?\s*(?:sec(?:ond)?s?\s*\/\s*100\s*(?:yd|yard)s?|sec(?:ond)?s?\s*\/\s*mi|%)(?:\s+(?:lower|slower))?[^.;]*?(?:if needed|intended effort)?/gi,
        ""
      )
      .replace(/;?\s*(?:reduce|decrease)\s+(?:the\s+)?(?:pace|power|target|intensity)[^.;]*/gi, "")
      .replace(/[.;\s]+$/, "");
  const stripRecoveryIncrease = (input) =>
    input
      .replace(
        /;?\s*(?:maintain the target and\s*)?(?:increase|add|allow up to)\s+(?:the\s+)?(?:rest|recovery)[^.;]*/gi,
        ""
      )
      .replace(
        /;?\s*(?:increase|add)\s+(?:no more than|up to)?\s*\d+\s*(?:seconds?|secs?|sec)\s*(?:of\s*)?(?:rest|recovery)[^.;]*/gi,
        ""
      )
      .replace(/[.;\s]+$/, "");
  const recoveryAllowance = value.match(
    /(?:add no more than|allow up to\s*\+?)\s*(\d+)\s*(?:seconds?|secs?|sec)\s*(?:of\s*)?(?:recovery|rest)\b/i
  );
  if (targetIsPrimary) {
    const seconds = Number(
      recoveryAllowance?.[1] || (String(workout.sport).toLowerCase() === "swim" ? 10 : 30)
    );
  ...
  if (/\b(?:allow|may|maximum|max\.?|up to|increase|reduce|slower)\b/i.test(value))
    return `${value}.`;
  ...
  if (sport === "bike")
    return `${value}; keep the written recovery and allow up to 5% lower power if needed.`;
  if (sport === "run")
    return `${value}; keep the written recovery and allow up to 10 sec/mi slower if needed.`;
...
export function normalizeDecision(decision, workouts) {
  ...
  const normalized = workouts.map((workout) => {
    ...
    const action =
      requestedAction !== "follow_as_written" && Object.keys(patch).length
        ? requestedAction
        : "follow_as_written";
    return {
      workout_id: String(workout.id),
      ...
      sport: workout.sport,
      original,
      priority: ["key", "supporting", "recovery"].includes(raw.priority)
        ? raw.priority
        : "supporting",
      target_flexibility: String(
        raw.target_flexibility || "Treat targets as a range and preserve the intended effort."
      ),
      action,
      proposed_change:
        action === "follow_as_written"
          ? null
          : String(raw.proposed_change || "Apply the displayed coaching adjustment."),
      reason: String(raw.reason || "Available evidence does not support a larger change."),
      execution_guidance: {
        target_ranges: (Array.isArray(raw.execution_guidance?.target_ranges) &&
        raw.execution_guidance.target_ranges.length
          ...
          : raw.execution_guidance?.target_range
            ...
            : baseGuidance(action, workout).target_ranges
        ).map((range) =>
          action === "follow_as_written" ? String(range) : ensureAdjustment(range, workout)
        ),
      },
      patch,
  ...
  const changesProposed = normalized.some(
    (item) => item.action !== "follow_as_written" && Object.keys(item.patch).length
  );
  ...
  const summary =
    evidenceStatus === "insufficient"
      ...
      : changesProposed
        ? String(decision?.summary || "A training adjustment is proposed for today.")
        : "Proceed as planned";
  return {
    summary,
    notification_summary: notificationSummary(normalized, changesProposed),
    reason: String(
      decision?.reason || "Today’s plan fits the available execution and recovery evidence."
    ),
    athlete_metrics: {
      ...
      resting_heart_rate: String(
        decision?.athlete_metrics?.resting_heart_rate ||
          "No current resting-heart-rate reading is available."
      ),
    },
    relevant_observations: Array.isArray(decision?.relevant_observations)
      ? decision.relevant_observations.slice(0, 8).map(String)
      : [],
    workouts: normalized,
...
export function reviewConversationText(review) {
  const verdict =
    review.evidence_status === "insufficient"
      ...
      : review.changes_proposed
        ...
        : "Go";
  const lines = [
    `**Today’s recommendation: ${verdict}**`,
    "",
    `**${review.summary}**`,
    "",
    review.reason,
  ...
  if (review.relevant_observations.length)
    lines.push(
      "",
      "## Why this matters",
      "",
      ...review.relevant_observations.map((item) => `- ${item}`)
    );
  for (const workout of review.workouts) {
    lines.push(
      ...
      `## ${workout.title}`,
      "",
      `**${workout.action === "follow_as_written" ? "Plan" : "Proposed change"}:** ${workout.proposed_change || "Follow the session as planned."}`
    );
    if (workout.action !== "follow_as_written") lines.push("", workout.reason);
    lines.push(
      "",
      "### How to execute",
      "",
    ...
    if (workout.evidence?.published?.length) {
      lines.push(
        "",
        "### Evidence",
        "",
        ...workout.evidence.published.map(
          (item) => `- ${item.source_id} / ${item.passage_id}: ${item.claim}`
        )
      );
    } else if (workout.evidence) {
      lines.push(
        "",
        "### Evidence",
        "",
        "- Published source: none used; this is explicitly coaching judgment."
      );
    }
    if (workout.evidence) {
      lines.push(
        `- **Athlete data:** ${workout.evidence.athlete_data.map((item) => `${item.fact_id} (${item.date})`).join(", ")}`,
        `- **Reasoning:** ${workout.evidence.reasoning}`,
        `- **Coaching judgment:** ${workout.evidence.coaching_judgment}`
      );
    }
  }
  return lines.join("\n");
...
export function createDailyReview({
  ...
  const review = {
    ...
    status:
      normalized.evidence_status === "insufficient"
        ? "evidence_insufficient"
        : normalized.changes_proposed
          ? "pending_approval"
          : "proceed_as_planned",
    decision: null,
  ...
  if (
    metrics.some(
      ([oldValue, newValue, threshold]) =>
        ...
        Math.abs(Number(oldValue) - Number(newValue)) >= threshold
    )
  )
    return true;
...
export function workoutHasStarted(workout, now = new Date()) {
  if (
    workout.status === "completed" ||
    Number(workout.actualDurationMinutes ?? workout.completed_data?.duration_minutes ?? 0) > 0
  )
    return true;

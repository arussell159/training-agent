...
test("today includes every scheduled workout and retains exact prescriptions", () => {
  const today = {
    ...
    title: "Bike Over-Unders",
    planned: { duration_minutes: 75, power_watts: 220 },
    structure: "original steps",
  };
  const context = coachTrainingContext(
    {
      ...
      planned: [
        today,
        { id: "2", workout_date: "2026-09-15", title: "Brick Run" },
        { id: "3", workout_date: "2026-09-16", title: "Tomorrow" },
      ],
      metrics: { fatigue: 91 },
    },
    "2026-09-15"
...
test("session context includes only today, tomorrow, and the previous eight days", () => {
  const context = coachTrainingContext(
    {
      history: ["2026-09-06", "2026-09-07", "2026-09-14", "2026-09-15"].map((workout_date) => ({
        workout_date,
      })),
  ...
  assert.deepEqual(
    context.recent_training_history.map((workout) => workout.workout_date),
    ["2026-09-07", "2026-09-14"]
  );
  assert.deepEqual(
    context.tomorrow_workouts.map((workout) => workout.workout_date),
    ["2026-09-16"]
  );

...
export async function createIntervalsWorkoutEvent(request, input = {}) {
  const name = String(input.name || "").trim();
  if (!name) throw new Error("The workout needs a name");
  const date = validDate(String(input.date || ""));
  const type = String(input.type || "").trim();
  if (!["Ride", "VirtualRide", "Run", "Swim", "WeightTraining", "Workout"].includes(type))
    throw new Error("Choose a supported workout type");
  const movingTime = Math.round(Number(input.moving_time));
  if (!(movingTime > 0) || movingTime > 24 * 3600)
    throw new Error("Workout duration must be between 1 second and 24 hours");
  if (
    !input.workout_doc ||
    typeof input.workout_doc !== "object" ||
    Array.isArray(input.workout_doc)
  )
    throw new Error("Every scheduled workout needs structured Intervals.icu workout data");
  if (!Array.isArray(input.workout_doc.steps) || !input.workout_doc.steps.length)
    throw new Error("Structured workout data needs at least one step");
  const structuredDuration = Number(input.workout_doc.duration);
  if (Number.isFinite(structuredDuration) && Math.abs(structuredDuration - movingTime) > 60)
    throw new Error("Structured workout duration does not match the planned duration");
  const body = {
    category: "WORKOUT",
    type,
    name,
    ...
    ...(String(input.description || "").trim()
      ...
      : {}),
    ...(Number.isFinite(Number(input.load_target))
      ? { load_target: Number(input.load_target) }
      : {}),
    ...(input.workout_doc && typeof input.workout_doc === "object"
      ? { workout_doc: input.workout_doc }
      : {}),
    ...(input.external_id ? { external_id: String(input.external_id) } : {}),
  };
  if (body.external_id) {
    const existing = await request(`/athlete/0/events?oldest=${date}&newest=${date}`);
    const match = (existing || []).find((event) => event.external_id === body.external_id);
    if (match) {
      if (match.name !== body.name || String(match.start_date_local).slice(0, 10) !== date)
        throw new Error("The workout operation ID is already attached to a different event");
      const verified = await request(`/athlete/0/events/${match.id}`);
      if (
        verified.category !== "WORKOUT" ||
        verified.name !== body.name ||
        String(verified.start_date_local).slice(0, 10) !== date ||
        Number(verified.moving_time || 0) !== movingTime ||
        !Array.isArray(verified.workout_doc?.steps) ||
        !verified.workout_doc.steps.length
      )
        throw new Error("Intervals.icu did not verify the existing structured workout");
      return verified;
    }
  }
  const created = await request("/athlete/0/events", {
  ...
  if (!created?.id) throw new Error("Intervals.icu did not confirm workout creation");
  const verified = await request(`/athlete/0/events/${created.id}`);
  if (
    verified.category !== "WORKOUT" ||
    verified.name !== body.name ||
    ...
    throw new Error("Intervals.icu did not verify the created workout");
  if (Number(verified.moving_time || 0) !== movingTime)
    throw new Error("Intervals.icu returned a different workout duration");
  if (!Array.isArray(verified.workout_doc?.steps) || !verified.workout_doc.steps.length)
    throw new Error("Intervals.icu did not retain the structured workout steps");
  return verified;
}
...
export async function applyIntervalsPatch(request, id, patch) {
  ...
  if (date < athleteLocalDate(new Date(), athlete.timezone || "America/Chicago"))
    throw new Error("Past workouts cannot be revised through a saved daily review");
  ...
  if (patch.structure != null || patch.coachComments != null)
    throw new Error(
      "Legacy structured review patches cannot be applied to Intervals.icu. Request a fresh workout description."
    );
  if (!Object.keys(body).length) throw new Error("No supported Intervals.icu change");
  await request(path, { method: "PUT", body: JSON.stringify(body) });
  const verified = await request(path);
  if (Object.entries(body).some(([k, v]) => verified[k] !== v))
    throw new Error("Intervals.icu did not confirm the change. Refresh before retrying.");

...
test("unique same-day named swim pairs without upstream links and retains the plan and actual", async () => {
  ...
  const activity = {
    ...
    type: "Swim",
    name: event.name,
    start_date_local: "2026-09-15T16:22:49",
    moving_time: 2930,
    distance: 2651.76,
  };
...
test("inferred pairing refuses ambiguous, different-day, different-sport and note matches", () => {
  ...
  for (const change of [{ start_date_local: "2026-09-14" }, { type: "Run" }, { name: "Other" }])
    assert.equal(pairIntervalsWorkouts([e], [{ ...a, ...change }]).size, 0);
  ...
  assert.equal(
    pairIntervalsWorkouts([{ ...e, name: "Different" }], [{ ...a, paired_event_id: 1 }]).get("1")
      .id,
    "a1"
  );
});
...
test("historical and paired completed workouts retain actual Intervals summary values", () => {
  const activity = {
    ...
    total_elevation_loss: 141,
    icu_training_load: 76,
    icu_intensity: 52.8,
    icu_joules: 764608,
    icu_average_watts: 126,
    average_heartrate: 151,
    max_heartrate: 171,
  };
  const historical = mapIntervalsWorkout(activity, "2026-09-15", null, true).workout_summary;
  const paired = mapIntervalsWorkout(
    { id: 42, start_date_local: "2026-09-06T08:00:00", type: "Ride", moving_time: 6000 },
    "2026-09-15",
    activity
  ).workout_summary;
  ...
  assert.equal(historical.completed.intensity_factor, 0.528);
...
test("app descriptions hide device definitions and use plain workout section headings", () => {
  ...
  const mapped = mapIntervalsWorkout(
    { id: 42, start_date_local: "2026-09-16T00:00:00", type: "Ride", description },
    "2026-09-16"
  );
  assert.equal(
    mapped.details,
...
test("app structure retains repeat sets and normalizes parsed yard distances for hover details", () => {
    ...
    steps: [
      {
        reps: 3,
        steps: [
          { distance: 400, duration: 439, pace: { value: 100, units: "secs" } },
  ...
  const mapped = mapIntervalsWorkout(
    { id: 43, start_date_local: "2026-09-16T00:00:00", type: "Swim", workout_doc },
    "2026-09-16"
  );
...
test("browsing a historical week still uses current Intervals fitness metrics", async () => {
  const context = await fetchIntervalsContext(
    async (path) =>
      path === "/athlete/0"
        ? { id: "i1", timezone: "America/Chicago" }
        : path.includes("oldest=2026-09-15")
          ? [{ id: "2026-09-15", ctl: 61, atl: 70 }]
          : path.includes("/wellness")
            ? [{ id: "2026-08-20", ctl: 20, atl: 30 }]
            : [],
    { now: new Date("2026-09-15T18:00:00Z"), range: { start: "2026-08-17", end: "2026-08-23" } }
  );
  ...
  const request = createIntervalsClient(
    ...
    async (url, options) => {
      ...
      assert.equal(
        ...
        `Basic ${Buffer.from("API_KEY:fixture-key").toString("base64")}`
      );
      return {
        ok: true,
        text: async () =>
          JSON.stringify({
            id: "i1",
            api_key: "private",
            nested: { access_token: "private", ftp: 250 },
          }),
      };
    }
  );
  assert.deepEqual(await request("/athlete/0"), { id: "i1", nested: { ftp: 250 } });
  const failed = createIntervalsClient({ INTERVALS_API_KEY: "fixture-key" }, async () => ({
    ok: false,
    status: 401,
  }));
  await assert.rejects(
    failed("/athlete/0"),
    (error) => error.status === 401 && !error.message.includes("fixture-key")
  );
});
...
test("context maps seconds and m/s, deduplicates paired activities and uses athlete timezone", async () => {
  const data = {
    "/athlete/0": {
      ...
      sportSettings: [
        { types: ["Ride"], ftp: 250 },
        { types: ["Run"], threshold_pace: 3.42, lthr: 170 },
        { types: ["Swim"], threshold_pace: 1 },
      ],
    },
    activities: [
      {
        ...
        type: "Ride",
        name: "Completed",
        start_date_local: "2026-09-14T08:00:00",
        moving_time: 3600,
        icu_training_load: 50,
      },
      { id: "a2", type: "Run", start_date_local: "2026-09-15T08:00:00", moving_time: 1800 },
    ],
    events: [
      {
        id: 1,
  ...
  assert.equal(context.athlete.zones.run_threshold_pace, "7:51 min/mi");
...
test("calendar move preserves time, multiday span and prescription using date-only patch", async () => {
  let stored = {
    id: 1,
    start_date_local: "2026-09-15T08:30:00",
    end_date_local: "2026-09-17T08:30:00",
    description: "- 20m 60%",
    moving_time: 1200,
  };
...
test("invalid calendar IDs, invalid dates and unconfirmed moves cannot report success", async () => {
  ...
  for (const date of ["2026-02-30", "2026-99-99", "bad"])
    assert.throws(() => validDate(date), /valid calendar date/);
  await assert.rejects(
    moveIntervalsEvent(
      async () => ({ start_date_local: "2026-09-15T00:00:00" }),
      "event:1",
      "2026-09-20"
    ),
    /did not confirm/
...
test("copy strips pairing and identity; deletion requires a verified 404", async () => {
  ...
  const request = async (path, options) => {
    if (options?.method === "POST") {
      ...
      assert.equal(body.paired_activity_id, undefined);
  ...
  await assert.rejects(
    changeIntervalsEvent(async () => original, "event:1", "delete"),
    /did not confirm/
  );
});

test("race creation uses the Intervals race category and verifies the saved event", async () => {
  let stored = null;
  const request = async (path, options) => {
    if (path.includes("?oldest=")) return stored ? [stored] : [];
    if (options?.method === "POST") {
      assert.equal(path, "/athlete/0/events");
      stored = { id: 77, ...JSON.parse(options.body) };
      return stored;
    }
    assert.equal(path, "/athlete/0/events/77");
    return stored;
  };
  const event = await createIntervalsRaceEvent(request, {
    name: "Goal Race",
    date: "2027-05-16",
    priority: "A",
    externalId: "annual-plan:test",
  });
  assert.equal(event.category, "RACE_A");
  assert.equal(event.start_date_local, "2027-05-16T00:00:00");
  assert.equal(event.external_id, "annual-plan:test");
  assert.equal(
    (
      await createIntervalsRaceEvent(request, {
        name: "Goal Race",
        date: "2027-05-16",
        priority: "A",
        externalId: "annual-plan:test",
  ...
  await assert.rejects(
    createIntervalsRaceEvent(request, { name: "", date: "2027-05-16", priority: "A" }),
    /race name/
  );
});

test("race updates change the name, priority and date and verify the provider result", async () => {
  let stored = {
    id: 77,
    name: "Goal Race",
    category: "RACE_A",
    start_date_local: "2027-05-16T00:00:00",
    end_date_local: "2027-05-17T00:00:00",
  };
  const request = async (path, options) => {
    assert.equal(path, "/athlete/0/events/77");
    if (options?.method === "PUT") stored = { ...stored, ...JSON.parse(options.body) };
    return stored;
  };
  const event = await updateIntervalsRaceEvent(request, "event:77", {
    name: "Updated Race",
    date: "2027-05-23",
    priority: "B",
  });
  assert.equal(event.name, "Updated Race");

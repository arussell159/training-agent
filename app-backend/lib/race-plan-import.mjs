...
const bikeNotes='Plan FTP: 203W. Cadence 85–95 rpm unless specified. Stay in the aerobars. Easy valleys and warm down provide recovery; adjacent efforts are continuous.';
const runNotes='Use the plan pace ranges, not faster targets. Easy sections provide recovery. Shorten efforts if the shins flare; do not push through pain.';
const swimNotes='25-yard pool. Z1 easy; Z2 race effort; Z3 steady; Z4 strong; Z5 CSS. Rest is seconds on the wall, not a send-off. Unlisted pauses are continuous (0 secs rest).';
const plans=[];
const B=(d,n,w,m,c,notes='')=>plans.push(workout(d,'Ride',n,w,m,c,bikeNotes+' '+notes));
const R=(d,n,w,m,c,notes='')=>plans.push(workout(d,'Run',n,w,m,c,runNotes+' '+notes));
const S=(d,n,w,m,c)=>plans.push(workout(d,'Swim',n,w,m,c,swimNotes));
const brick=(d,a,b,c)=>R(d,'Race-Pace Brick Run',[rp(a)],[rp(b,race)],[rp(c)],'Start immediately after the bike and time the transition.');

B('2026-09-15','Race-Power Over-Unders',[ramp(10),repeat(3,power(30,173),power(30,106)),bp(2,106)],
 [repeat(3,bp(3,148),bp(2,166)),bp(4,106),repeat(4,bp(2,146),power(90,173)),bp(4,106),bp(6,152),bp(2,183),bp(4,142)],[cool(11)]);brick('2026-09-15',5,5,5);
R('2026-09-16','Threshold and Steady Run',[rp(15)],[repeat(3,rp(6,'8:50-9:00'),rp(2)),rp(3),repeat(2,rp(4,'9:30-9:40'),rp(2))],[rp(6)]);
S('2026-09-17','CSS Swim Repeats',[swim(300,1,'free with fins'),sw(4,50,1,10,'drill'),sw(4,50,4,15,'build')],[repeat(4,sw(3,100,5,20),swim(50,1),rest(30))],[swim(100,1)]);
R('2026-09-17','Easy Aerobic Run',[],[rp(30)],[]);
B('2026-09-19','Race-Course Bike Rehearsal',[ramp(15)],[repeat(4,bp(8,154),bp(2,146)),bp(5,106),repeat(3,bp(6,148),bp(1,173),bp(3,132)),bp(5,106),{...bp(10,126),cadence:{value:100,units:'rpm'}},bp(20,148),bp(5,138),bp(5,106),repeat(2,bp(10,152),bp(2,138))],[cool(21)],'Rehearse race nutrition. Hard cap 154W on the climb. Outdoors on rolling roads preferred.');brick('2026-09-19',5,20,5);
R('2026-09-20','Long Run with Race-Pace Blocks',[rp(30)],[repeat(3,rp(15,race),rp(5))],[rp(15)]);
S('2026-09-21','Race-Rhythm Swim',[swim(300,1,'free with fins'),repeat(4,swim(25,1,'drill'),swim(25,1)),sw(4,50,4,15,'descend 1–4')],[repeat(2,swim(500,2),rest(30),swim(200,4),rest(30),swim(100,1),rest(20))],[swim(100,1)]);
B('2026-09-22','Race-Power Surges',[ramp(10),repeat(4,power(20,193),power(40,106)),bp(2,106)],[repeat(4,power(150,148),power(30,179)),bp(3,106),repeat(3,bp(3,152),bp(1,132)),bp(3,106),{...bp(6,146),cadence:{value:95,units:'rpm'}}],[cool(8)]);brick('2026-09-22',3,8,4);
R('2026-09-23','Progressive Interval Run',[rp(12)],[rp(6,'9:30-9:40'),rp(2),rp(5,'9:10-9:20'),rp(2),rp(4,'8:50-9:00'),rp(2),rp(3),repeat(2,rp(3,race),rp(1))],[rp(6)]);
S('2026-09-24','Sighting and Race-Effort Swim',[swim(300,1,'free with fins'),sw(4,50,1,10,'drill')],[sw(6,150,4,20,'sight every 6th stroke on middle 50'),sw(4,100,2,15)],[swim(200,1)]);
R('2026-09-24','Easy Aerobic Run',[],[rp(25)],[]);
B('2026-09-26','Progressive Race-Power Ride',[ramp(15)],[bp(20,146),bp(5,106),bp(15,152),bp(5,106),bp(10,158),bp(5,106),repeat(3,bp(4,162),bp(2,142)),bp(5,106),repeat(2,bp(8,148),bp(2,132))],[cool(17)]);brick('2026-09-26',3,12,5);
R('2026-09-27','Progressive Long Run',[rp(25)],[rp(20,'10:10-10:20'),rp(20,race)],[rp(10)]);
S('2026-09-28','Race-Week Swim Tune-Up',[swim(300,1,'free with fins'),repeat(4,swim(25,1,'drill'),swim(25,1))],[sw(6,100,4,20),sw(4,50,5,20),swim(300,2)],[swim(200,1)]);
B('2026-09-29','Race-Week Bike Activation',[ramp(10)],[repeat(3,bp(3,148),bp(2,106)),bp(3,106),repeat(4,bp(1,183),bp(2,106))],[cool(10)]);brick('2026-09-29',4,4,2);
R('2026-09-30','Race-Pace Run Activation',[rp(12)],[repeat(3,rp(3,race),rp(2))],[rp(8)],'Wear race shoes and socks.');
S('2026-10-01','Pre-Race Swim Activation',[swim(300,1,'free with fins'),sw(4,50,4,15,'build')],[sw(6,50,5,20),swim(200,2)],[swim(200,1)]);
B('2026-10-03','Pre-Race Bike Shakeout',[{duration:600,power:{start:112,end:126,units:'w'}}],[repeat(3,bp(1,152),bp(2,106))],[cool(6)]);
R('2026-10-03','Pre-Race Run Shakeout',[rp(4)],[repeat(3,run(20,race),run(60))],[rp(2)],'10 min total; the unallocated easy time in the source is distributed as 4 min warm up, 60 secs recovery per rep, and 2 min warm down.');
...
export async function importRacePlan(apply=false){
 ...
 const store=createSupabaseSettingsStore(bootstrap);const config={...bootstrap,...await store.read()};const request=createIntervalsClient(config);
 const settings=await request('/athlete/0/sport-settings');
 const patches=[['Ride',{ftp:203}],['Run',{threshold_pace:1609.344/535,pace_units:'MINS_MILE',workout_order:'PACE_HR_POWER'}],['Swim',{threshold_pace:91.44/93,pace_units:'SECS_100Y',pace_zones:[93/108*100,93/102*100,93/98*100,93/95*100,999],pace_zone_names:['Easy','Race effort','Steady','Strong','CSS'],workout_order:'PACE_HR_POWER'}]];
 if(!apply)return {count:plans.length,patches,plans:plans.map(p=>({date:p.start_date_local,name:p.name,type:p.type,totals:totals(p.workout_doc.steps),description:p.description}))};
 const equal=(a,b)=>typeof b==='number'?Number.isFinite(a)&&Math.abs(a-b)<.0001:Array.isArray(b)?Array.isArray(a)&&a.length===b.length&&b.every((x,i)=>equal(a[i],x)):a===b;
 for(const [type,patch] of patches){const s=settings.find(s=>s.types.includes(type));if(!s)throw new Error('Missing sport settings '+type);await request(`/athlete/0/sport-settings/${s.id}`,{method:'PUT',body:JSON.stringify(patch)});const verified=await request(`/athlete/0/sport-settings/${s.id}`);for(const [key,value] of Object.entries(patch)){if(!equal(verified[key],value))throw new Error('Settings verification failed '+type+' '+key);}}
 const existing=await request('/athlete/0/events?oldest=2026-09-15&newest=2026-10-04');const imported=[];
 for(const p of plans){const match=existing.find(e=>e.external_id===p.external_id);if(!match && existing.some(e=>e.start_date_local===p.start_date_local && e.type===p.type))throw new Error('Possible scheduled duplicate '+p.start_date_local+' '+p.type);const {workout_doc,...metadata}=p;
 ...
 description:'Race overview from supplied plan; not a mixed-sport device workout.\n\nSwim: 1.2 mi at Z2, target 37–39 min. Controlled first 200 yd, sight every 6–8 strokes.\nT1: under 5 min.\nBike: 56 mi at 142–152w, cap 154w on climb and 160w on short rises; flat finish 142–148w. Last 10 min at 126–132w, 90+ rpm. Target 2:55–3:00.\nT2: under 4 min.\nRun: 13.1 mi. First 2 mi at 10:10–10:20 min/mile, miles 3–11 at 9:50–10:00 min/mile, final 2.1 mi by effort. Target 2:08–2:12.\n\nGoal: sub-6 hours; plan split calculation 5:55.'};
 const priorRace=existing.find(e=>e.external_id===raceEvent.external_id);
 const createdRace=await request(priorRace?`/athlete/0/events/${priorRace.id}`:'/athlete/0/events',{method:priorRace?'PUT':'POST',body:JSON.stringify(raceEvent)});
 const verifiedRace=await request(`/athlete/0/events/${createdRace.id}`);
 if(verifiedRace.category!=='RACE_A' || verifiedRace.description!==raceEvent.description)throw new Error('Race overview verification failed');
 imported.push({id:createdRace.id,date:raceEvent.start_date_local,type:raceEvent.type,name:raceEvent.name});
 await store.save({RACE_PLAN_IMPORT_REPORT:JSON.stringify({source:'Alex_Russell_Triathlon_Sep-Oct (3).docx',imported,imported_at:new Date().toISOString()})});
 return {imported};
}

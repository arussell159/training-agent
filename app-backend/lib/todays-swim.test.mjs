...
test("FIT pool metadata distinguishes yards from meters while storing pool length in meters", () => {
  const payload = Buffer.from([
    0x40, 0, 0, 26, 0, 3, 4, 1, 0, 14, 2, 0x84, 15, 1, 0, 0, 5, 0xee, 0x08, 1,
  ]);
  ...
  assert.deepEqual(readFitWorkoutMetadata(fit), [
    { sport: 5, poolLengthMeters: 22.86, poolLengthUnit: "yards" },
  ]);
  fit[fit.length - 1] = 0;
...
test("FIT repetition control records expand work and rest the correct number of times", () => {
  ...
  const result = expandFitWorkoutSteps([
    work,
    rest,
    { index: 2, durationType: 6, durationValue: 0, targetValue: 8 },
  ]);
  assert.equal(result.length, 16);
  assert.equal(result.filter((s) => s.intensity === 1).length, 8);
  assert.equal(
    result.filter((s) => s.durationType === 1).reduce((sum, s) => sum + s.durationValue, 0),
    8 * 9144
  );
});

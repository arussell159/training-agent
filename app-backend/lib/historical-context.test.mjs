...
test("TrainingPeaks archive remains visible without an Intervals connection and is read-only", () => {
  ...
  const archive = {
    ...
    history: [past],
    planned: [past, { ...past, id: 13, workout_date: "2026-09-17" }],
  };
  const result = mergeHistoricalContext(
    { source: "not-connected", history: [], planned: [] },
    archive,
    "2026-09-15"
  );
...
test("Intervals history wins exact matches without hiding other same-day workouts", () => {
  ...
  const result = mergeHistoricalContext(
    { source: "intervals", history: [live], planned: [] },
    { source: "trainingpeaks", history: [old, { ...old, id: 13, title: "Evening run" }] },
    "2026-09-15"
  );
  assert.deepEqual(
    result.history.map((w) => w.id),
    ["activity:42", "tp-history:13"]
  );
...
test("Wellness archive preserves details while newer Intervals fields override values", () => {
  ...
  const result = mergeHistoricalContext(
    { history: [], planned: [], wellness_history: [{ date: "2026-09-15", hrv: 64 }] },
    { source: "trainingpeaks", wellness_history: [{ timeStamp: "2026-09-15T00:00:00", details }] },
    "2026-09-15"
  );

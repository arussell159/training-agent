...
export function coachTrainingContext(context, currentDate) {
  ...
  const workouts = new Map([...history,...planned].map((workout,index) => [String(workout.id || `${workout.workout_date}:${index}`),workout]));
  return {
    current_date:currentDate,
    athlete:context.athlete,
    today_workouts:[...workouts.values()].filter(workout => String(workout.workout_date || '').slice(0,10) === currentDate),
    metrics:context.metrics,
    wellness:context.wellness,
    tomorrow_workouts:planned.filter(workout => String(workout.workout_date || '').slice(0,10) === tomorrow),
    recent_training_history:history.filter(workout => workout.workout_date >= historyStart && workout.workout_date < currentDate),

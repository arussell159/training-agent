...
export function createCoach({ source, fetchImpl = fetch, now = Date.now }) {
  return async function answer({
    ...
    const instructions = [
      ...
      reportContext?.target.kind === "pre"
        ? "For this pre-workout report the server has verified today’s overnight sleep and prior-day data in a snapshot exported on the athlete’s local today. That morning snapshot remains valid for today’s planned workout even when it is more than an hour old. Use the supplied dated overnight metrics and start-of-day readiness values; do not refuse or demand a new sync solely because hours have elapsed. Still check metric dates and follow same-day continuation rules if a completed session actually exists."
      ...
      ...(reportContext
        ? [
            ...
            "STRICT REPORT OUTPUT: Follow the supplied template's field labels, order, conditional gates and sentence counts exactly. The report_markdown must contain ONLY the filled report body: zero preamble, extra commentary, greetings, sign-off, suggestions outside the template, template instructions, placeholder brackets or code fences. Presentation: plain text lines with only each field label and its colon optionally bolded (for example **RHR:**). Keep section titles as ordinary text. No Markdown headings, tables, horizontal rules, decorative bullets, bold values or other layout instructions. Use a separate line for each field and preserve the exact labels. Do not invent extra headings or change the template to a generic essay. Template Interpretation and other explicitly requested notes are part of the report, not extra commentary. Apply ALL conditional omissions, sport eligibility, DFA a1 depth/comparator gates, and evidence rules in the template. Use display.* values for preferred units and _formatted source fields for durations/sleep. Never reuse numbers from older reports. For post-workout reports provide exactly one Completed workout block per same-day activity ID, in start-time order, including secondary activities. For same-day pre-workout continuation use ONLY the replacement continuation block; current state comes from reportEvidence.currentStateCheckIn, never from an earlier activity's Feel. If essential evidence is missing, return complete=false instead of saving a partial report. For weekly/block summary return exactly one concise sentence based only on this report's Interpretation, without a label, Markdown, new facts or extra advice. This separate summary is a preview and must NOT be appended to the report body. For pre/post summary must be null.",
          ]
        : []),

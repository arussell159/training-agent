...
const block = {
  ...
  phase: "Peak",
  weeks: [
    { start_date: "2026-09-07", end_date: "2026-09-13", phase: "Peak" },
    { start_date: "2026-09-14", end_date: "2026-09-20", phase: "Peak" },
  ],
};
...
test("block source coverage required and missing observations remain unavailable", () => {
  ...
  const text = buildBlockReport(block, {
    generated_at: "now",
    weekly_180d: block.weeks.map((w) => ({
...
test("block append preserves weekly text, verifies ambiguous write, and does not regenerate", async () => {
  let note = {
    id: 1,
    external_id: "section11-weekly-report-2026-09-14",
    description: "Existing weekly report",
  };
  let writes = 0;
  const request = async (_, options) => {
    if (!options) return [note];
    writes++;
    note = { ...note, ...JSON.parse(options.body) };
    throw new Error("Network interrupted after write");
  };

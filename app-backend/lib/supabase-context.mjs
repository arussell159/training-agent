  ...
  return {
    ...
    async getContext(athleteId = "default") {
      ...
      const [configRows, workouts, comments] = await Promise.all([
        ...
        request("workout_context", {
          query: `?athlete_id=eq.${athleteId}&workout_date=gte.${cutoff}&order=workout_date.desc&select=*`,
        }),
        request("athlete_comments", {
          query: `?athlete_id=eq.${athleteId}&created_at=gte.${cutoff}&order=created_at.desc&select=*`,
        }),
      ]);
    ...
    async updateNotificationPreferences(athleteId = "default", patch = {}) {
        ...
        body: JSON.stringify([
          {
            ...
            last_delivery_error: Object.hasOwn(patch, "last_delivery_error")
              ? patch.last_delivery_error
      ...
      return {
        ...current,
        ...(rows?.[0] || {}),
        review_time: String(rows?.[0]?.review_time || current.review_time).slice(0, 5),
      };
    },

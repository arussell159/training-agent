...
export const STORED_SETTINGS = [
  ...
  "APP_DATA",
  "HISTORICAL_ARCHIVE",
  "CALENDAR_SUMMARY_OPEN",
  "TRAININGPEAKS_IMPORT_REPORT",
  "RACE_PLAN_IMPORT_REPORT",
...
export function createSupabaseSettingsStore(bootstrap, fetchImpl = fetch) {
  ...
  if (!url || !secret)
    throw new Error("Configure the backend Supabase URL and secret before saving Settings.");
  ...
  if (material.length < 32)
    throw new Error("Settings encryption requires a backend secret of at least 32 characters.");
  ...
  function decrypt(name, value) {
    try {
      ...
      throw new Error(
        "Saved Settings could not be decrypted. Restore the original backend settings encryption key."
      );
    }
  }
  async function request(query, options = {}) {
    ...
    try {
      ...
      if (denied)
        throw new Error(
          "The backend is blocked from connecting to Supabase. Restart the local app server with network access, then retry."
        );
    ...
    if (!response.ok) {
      if (response.status === 404)
        throw new Error(
          "Supabase Settings table is missing. Run app-backend/supabase/settings.sql in the Supabase SQL editor."
        );
      // Never echo provider error bodies: they may contain submitted credentials or encrypted values.
      if (response.status === 401)
        throw new Error(
          "Supabase rejected the locally configured backend secret (401). Update the project URL and secret together in Settings → Supabase. Your Intervals.icu key was not rejected by this error."
        );
      throw new Error(
        `Supabase Settings request failed (${response.status}). Check the backend Supabase configuration and table permissions.`
      );
    }
  ...
  return {
    async read(names = STORED_SETTINGS) {
      ...
      const rows = await request(
        `?scope=eq.${encodeURIComponent(scope)}&select=name,encrypted_value${filter}`
      );
      return Object.fromEntries(
        rows
          .filter((row) => selected.includes(row.name))
          .map((row) => [row.name, decrypt(row.name, row.encrypted_value)])
      );
    },
    async save(patch) {
      ...
      const rows = Object.entries(values).map(([name, value]) => ({
        scope,
        name,
        encrypted_value: encrypt(name, value),
    ...
    async read(names = STORED_SETTINGS) {
      ...
      try {
        const id = JSON.stringify([
          bootstrap.SUPABASE_URL,
          bootstrap.SETTINGS_SCOPE,
          bootstrap.SUPABASE_SECRET_KEY,
          bootstrap.SETTINGS_ENCRYPTION_KEY,
          names,
        ]);

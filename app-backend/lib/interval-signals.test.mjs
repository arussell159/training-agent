...
import {
  ...
  intervalSignals,
  tooltipPosition,
} from "../../ui/src/lib/interval-signals.ts";
...
test("swim pace and stroke rate are constant across each full recorded interval", () => {
  ...
  const result = intervalSignals(points, [
  ...
  assert.equal(
    result[0].lap.end - result[0].lap.start,
    4 * (result[1].lap.end - result[1].lap.start)
  );
  assert.equal(
    result.find((i) => 415 >= i.lap.start && 415 < i.lap.end),
    undefined
  );
});
test("tooltip follows the inspection time horizontally and stays within the chart edges", () => {
  assert.match(tooltipPosition(0, 100), /10%/);
  assert.match(tooltipPosition(50, 100), /53%/);
  assert.match(tooltipPosition(100, 100), /96%/);
  assert.match(tooltipPosition(0, 0), /clamp/);
  assert.equal(tooltipPosition(200, 100), tooltipPosition(100, 100));
});

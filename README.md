...
The app's Intervals.icu sync runs the private repository's `sync.py` in a Vercel Python function and commits its JSON output directly to GitHub when source data changes or you press Refresh. Set these **server-only** environment variables in Vercel Production, then redeploy:
...
Section 11 reports are app-owned records in the backend-only Supabase `coach_reports` table. `/coach` lists Pre, Post, Weekly, and Block reports from that table. Pre/post titles include the sport and show the workout date; weekly/block cards show their date range. The authenticated `GET /api/coach/reports` and `GET /api/coach/reports/:id` endpoints read reports, and `POST /api/coach/reports` upserts by stable source key. Regenerating a report updates its row instead of adding another card.

Reports use the same exact persistent prompt, fresh private dossier and official Section 11 protocol as chat, plus the athlete's exact supplied template and the official hierarchy. Training data is pinned to its GitHub commit; supplied templates have independent content hashes. Field labels, ordering, conditional omissions, source units and interpretation lengths follow the templates, with no extra commentary. The Coach page groups Pre and Post reports by Monday week and displays every report's saved text verbatim in the details sheet. Same-day pre-workout continuation requires a fresh Feel/soreness/symptoms check-in before generation. Historical reports use dated history, not today's rolling metrics relabelled as an old period. Previously saved reports provide continuity to the next report level. Missing essential data or an incomplete model response leaves the report retryable; it does not mark an apology as completed.

Existing encrypted weekly/block records remain in `app_settings` as a recovery copy. On first catalog load after deployment, the backend copies them and historical Intervals.icu report notes into `coach_reports` without deleting either source. The catalog then reads only Supabase rows. New weekly/block generation saves to `coach_reports` and does not publish report notes or comments to Intervals.icu. Apply the `coach_reports` section of `app-backend/supabase/schema.sql` before deploying the new backend.

The private data repository's `.github/workflows/auto-sync.yml` already has its scheduled trigger disabled. Its manual dispatch remains available for recovery or a historical rebuild, but the app does not dispatch it. The copy in `app-backend/coach/github/auto-sync.yml` keeps that manual workflow and the `refresh_history` input. Direct sync needs **Contents: Read and write** on the application token.

**Direct sync:** The app starts a refresh on load, the **Refresh Intervals.icu** button starts one on demand, and saved edits also prompt a background check. The server invokes `sync.py` from the configured private data repository in an isolated Python function, loads the existing JSON files as its incremental cache, rebuilds history when it is over 28 days old, and commits `latest.json`, `history.json`, `intervals.json`, `routes.json`, `ftp_history.json`, `saved_workouts.json`, and an archive snapshot in one commit. This path does not dispatch GitHub Actions. It needs **Contents: Read and write** on the app's GitHub token, the Intervals.icu API key already configured in the app, and `sync.py` at the data repository root. Set `WEEK_START` and `ZONE_PREFERENCE` in Vercel too if the old workflow uses nondefault values. The Python function has a five-minute maximum duration and runs on Vercel; local `npm start` does not host it. Verify one direct refresh in production after deploying the app.
